...
def plot_ocp_solution(mpc):
    ...
    for time_idx in range(T):
        ...
        for fname in mpc.ee_frame_names:
            ...
            f.append(
                mpc.solver.problem.runningDatas[0]
                .differential.multibody.contacts.contacts[ct_frame_name]
                .f.linear
            )
    ...
    for i, frame_idx in enumerate(supportFeetIds):
        ...
        axs[i, 2].plot(
            time_lin, Fz_lb, linewidth=4, color="k", marker="o", alpha=0.25, label="lb"
        )
        axs[i, 0].grid()
        axs[i, 1].grid()
        axs[i, 2].grid()
...
class Go2MPCClassical:
    ...
    def initialize(
        ...
        self.supportFeetPos0 = np.array(
            [self.lfFootPos0, self.rfFootPos0, self.lhFootPos0, self.rhFootPos0]
        )
    ...
    def createProblem(self):
        ...
        for t in range(self.HORIZON + 1):
            self.contactModel = crocoddyl.ContactModelMultiple(
                self.ccdyl_state, self.nu
            )
            ...
            for i, frame_idx in enumerate(self.supportFeetIds):
                ...
                self.contactModel.addContact(
                    self.rmodel.frames[frame_idx].name + "_contact", support_contact
                )

            # Contact model for the EE
            arm_contact = crocoddyl.ContactModel3D(
                self.ccdyl_state,
                self.armEEId,
                self.armEEPos0,
            ...
            self.contactModel.addContact(
                self.rmodel.frames[self.armEEId].name + "_contact", arm_contact
            )
            ...
            if t != self.HORIZON:
                ...
                contact_force_residual = crocoddyl.ResidualModelContactForce(
                    self.ccdyl_state, self.armEEId, self.ef_des_force, 3, self.nu
                )
                contact_force_activation = crocoddyl.ActivationModelWeightedQuad(
                    np.array([1.0, 1.0, 1.0])
                )
                contact_force_track = crocoddyl.CostModelResidual(
                    self.ccdyl_state, contact_force_activation, contact_force_residual
                )
                costModel.addCost(
                    "contact_force_track", contact_force_track, self.F_ee_weight
                )
            ...
            constraintModelManager = crocoddyl.ConstraintModelManager(
                self.ccdyl_state, self.ccdyl_actuation.nu
            )
            if t != self.HORIZON:
                # Feet
                for frame_idx in self.supportFeetIds:
                    # Friction cones
                    name = self.rmodel.frames[frame_idx].name + "_contact"
                    residualFriction = friction_utils.ResidualFrictionCone(
                        self.ccdyl_state,
                        name,
                        self.friction_mu,
                        self.ccdyl_actuation.nu,
                        normal="z",
                    )
                    ...
                    residualForce = crocoddyl.ResidualModelContactForce(
                        self.ccdyl_state, frame_idx, pin.Force.Zero(), 3, self.nu
                    )
                    forceBoxConstraint = crocoddyl.ConstraintModelResidual(
                        self.ccdyl_state,
                        residualForce,
        ...
        self.ocp = crocoddyl.ShootingProblem(
            self.x0, self.running_models[:-1], self.running_models[-1]
        )
    ...
    def getSolution(self, k=None):
        ...
        for fname in self.ee_frame_names:
            ...
            data = self.solver.problem.runningDatas[
                0
            ].differential.multibody.contacts.contacts[ct_frame_name]
