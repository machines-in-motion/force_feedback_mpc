...
from Go2MPC_wrapper_classical import (
    Go2MPCClassical,
    getForceSensor,
    setGroundFriction,
)
...
if USE_MUJOCO:
    for i in range(N_SIMU):
        ...
        for action_model in m[:-1]:
            action_model.differential.costs.costs[
                "contact_force_track"
            ].cost.residual.reference.linear[:] = f_des_3d
        ...
        if WITH_INTEGRAL:
            ...
            if i % int(SIM_FREQ / MPC_FREQ) == 0:
                ...
                pin.computeAllTerms(
                    mpc.rmodel,
                    mpc.rdata,
                    mpc.xs[0][: mpc.rmodel.nq],
                    mpc.xs[0][mpc.rmodel.nq :],
                )
                J = pin.getFrameJacobian(
                    mpc.rmodel, mpc.rdata, mpc.armEEId, pin.LOCAL_WORLD_ALIGNED
                )
...
# PYBULLET SIMULATION
    ...
    for i in range(N_SIMU):
        ...
        for km, action_model in enumerate(m):
            action_model.differential.costs.costs[
                "contact_force_track"
            ].cost.residual.reference = pin.Force(f_des_3d, np.zeros(3))
        # Get state from simulation
