...
for k,m in enumerate(models):
    ...
    for col_idx in range(len(robot.collision_model.collisionPairs)):
        ...
        # m.g_lb = np.concatenate([m.differential.constraints.g_lb, np.array([-np.inf]) ])
        # m.g_ub = np.concatenate([m.differential.constraints.g_ub, np.array([np.inf]) ])
# wfhwef
# # Activate collisions initially + set bounds
# for col_idx in range(len(robot.collision_model.collisionPairs)):
#   for k in range(solver.problem.T):
#     solver.problem.runningModels[k].differential.constraints.constraints['collisionBox_' + str(col_idx)].active=True
#     solver.problem.runningModels[k].differential.constraints.constraints['collisionBox_' + str(col_idx)].constraint.updateBounds(
#                   np.array([0.]),
#                   np.array([np.inf]),)
#   solver.problem.terminalModel.differential.constraints.constraints['collisionBox_' + str(col_idx)].active=True
