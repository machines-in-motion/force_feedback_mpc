...
def solveOCP(q, v, solver, nb_iter, target_reach, TASK_PHASE, target_force):
    ...
    if TASK_PHASE == 3:
        for k in range(solver.problem.T + 1):
            ...
            m[k].differential.costs.costs["translation"].weight = 2.0
            m[k].differential.costs.costs["rotation"].active = True
            m[k].differential.costs.costs[
                "rotation"
            ].cost.residual.reference = pin.utils.rpyToMatrix(np.pi, 0, np.pi)
            # activate contact and force cost
            m[k].differential.contacts.changeContactStatus("contact", True)
            if k != solver.problem.T:
                fref = pin.Force(np.array([0.0, 0.0, target_force[k], 0.0, 0.0, 0.0]))
    ...
    if TASK_PHASE == 4:
        for k in range(solver.problem.T + 1):
            ...
            m[k].differential.costs.costs["translation"].weight = 60.0
            m[k].differential.costs.costs["rotation"].active = True
            m[k].differential.costs.costs[
                "rotation"
            ].cost.residual.reference = pin.utils.rpyToMatrix(np.pi, 0, np.pi)
...
for i in range(sim_data.N_simu):
    ...
    if i % int(sim_data.simu_freq / sim_data.plan_freq) == 0:
        ...
        # Solve OCP
        solveOCP(
            q, v, solver, config["maxiter"], target_position, TASK_PHASE, target_force
        )
    ...
    sim_data.record_simu_cycle_measured(
        i, x_mea_SIMU, x_mea_no_noise_SIMU, tau_mea_SIMU, f_mea_SIMU
    )
    # Display real
