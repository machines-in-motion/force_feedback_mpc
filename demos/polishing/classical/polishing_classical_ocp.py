...
def main(PLOT=False, DISPLAY=True):
    ...
    config = path_utils.load_yaml_file(
        os.path.dirname(os.path.realpath(__file__)) + "/polishing_classical.yml"
    )
    ...
    if WARM_START_IK:
        ...
        for k, m in enumerate(models):
            ...
            q_ws, v_ws, _ = pin_utils.IK_placement(
                robot, q_ws, id_endeff, Mref, DT=1e-2, IT_MAX=100
            )
            xs_init.append(np.concatenate([q_ws, v_ws]))
            if k < N_h:
                us_init.append(
                    pin_utils.get_tau(q_ws, v_ws, np.zeros((nq, 1)), f_ext, robot.model)
                )
    # Classical warm start using initial config
    else:
        f_ext = pin_utils.get_external_joint_torques(
            M_ee, config["frameForceRef"], robot
        )
    ...
    if DISPLAY:
        ...
        if len(config["contacts"]) != 0:
            ...
            if "force" in config["WHICH_COSTS"]:
                ...
                if force_axis == "z":
                    rotation_matrix = pin.rpy.rpyToMatrix(0.0, -np.pi / 2, 0.0)
                    # if force is along x axis in local frame , no transform
                elif force_axis == "x":
                    rotation_matrix = pin.rpy.rpyToMatrix(0.0, 0.0, 0.0)
                    # if force is along y axis in local frame , need to transform x-->y , i.e. +90° around z
                elif force_axis == "y":
                    rotation_matrix = pin.rpy.rpyToMatrix(0.0, 0.0, np.pi / 2)
                ct_frame_placement_aligned.rotation = (
                    ct_frame_placement_aligned.rotation.dot(rotation_matrix)
                )
        ...
        for i in range(N_h):
            ...
            if "force" in config["WHICH_COSTS"]:
                ...
                wrench = (
                    ...
                    .differential.multibody.contacts.contacts[
                        config["contacts"][0]["contactModelFrameName"]
                    ]
                    .f.vector
                )
                gui.resizeArrow(
                    "world/force",
                    real_size,
                    wrench_coef * np.linalg.norm(wrench[xyz[force_axis]]),
                )
