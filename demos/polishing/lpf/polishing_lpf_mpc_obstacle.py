...
def solveOCP(q, v, tau, solver, nb_iter, target_reach, TASK_PHASE, target_force):
    ...
    if TASK_PHASE == 3:
        for k in range(solver.problem.T + 1):
            ...
            m[k].differential.costs.costs["translation"].weight = 200.0
            m[k].differential.costs.costs["rotation"].active = True
            m[k].differential.costs.costs[
                "rotation"
            ].cost.residual.reference = pin.utils.rpyToMatrix(np.pi, 0, np.pi)
...
_, nonLpfStateIds = getJointAndStateIds(
    robot.model, list(set(robot.model.names[1:]) - set(lpf_joint_names))
)
...
EPSILON_INTERP = (1.0 / config["plan_freq"]) / config["dt"]
