...
def main(PLOT=False, DISPLAY=True):
    ...
    config = path_utils.load_yaml_file(
        os.path.dirname(os.path.realpath(__file__)) + "/polishing_lpf.yml"
    )
    ...
    u0 = pin_utils.get_tau(
        q0, v0, np.zeros((nq, 1)), f_ext, robot.model, config["armature"]
    )
    lpf_joint_names = robot.model.names[
        1:
    ]  # ['A1', 'A2', 'A3', 'A4'] # [] # ['A4'] #robot.model.names[1:]
    ...
    if WARM_START_IK:
        ...
        for k, m in enumerate(list(ocp.runningModels) + [ocp.terminalModel]):
            ...
            f_ext = pin_utils.get_external_joint_torques(
                Mref, config["frameForceRef"], robot
            )
            # Get joint state from IK
            q_ws, v_ws, eps = pin_utils.IK_placement(
                robot, q_ws, id_endeff, Mref, DT=1e-2, IT_MAX=100
            )
            tau_ws = pin_utils.get_tau(
                q_ws, v_ws, np.zeros((nq, 1)), f_ext, robot.model, config["armature"]
            )
    ...
    if PLOT:
        ...
        _, _ = ocp_data_handler.plot_ocp_results(
            ocp_data,
    ...
    if DISPLAY:
        ...
        ref_color = [1.0, 0.0, 0.0, 1.0]
        real_color = [0.0, 0.0, 1.0, 0.3]
        ct_color = [0.0, 1.0, 0.0, 1.0]
        ...
        if "contactModelFrameName" in config.keys():
            ...
            offset = 0.036
            ct_frame_placement.translation = ct_frame_placement.act(
            ...
            gui.addLandmark("world/contact_point", 0.25)
            ...
            if "force" in config["WHICH_COSTS"]:
                ...
                ct_frame_placement_aligned.rotation = (
                    ct_frame_placement_aligned.rotation.dot(
                        pin.rpy.rpyToMatrix(0.0, -np.pi / 2, 0.0)
                    )
                )
            ...
            if "friction" in config["WHICH_COSTS"]:
                mu = config["mu"]
                frictionConeColor = [1.0, 1.0, 0.0, 0.3]
                ...
                for k in range(m_generatrices.shape[1]):
                    l = robot.viewer.gui.addLine(
                        "world/cone_ray/" + str(k),
                        [0.0, 0.0, 0.0],
                        m_generatrices[:3, k].T.tolist()[0],
                        frictionConeColor,
                    )
                robot.viewer.gui.setScale("world/cone", [0.5, 0.5, 0.5])
        ...
        for i in range(N_h):
            ...
            # Move contact point
            m_ct = m_ee.copy()
            m_ct.translation = m_ct.act(np.array([0.0, 0.0, offset]))
            ...
            if "force" in config["WHICH_COSTS"]:
                ...
                wrench = (
                    ocp.runningDatas[i]
                    .differential.multibody.contacts.contacts["contact"]
                    .f.vector
                )
                gui.resizeArrow(
                    "world/force", real_size, wrench_coef * np.linalg.norm(wrench)
                )
                m_ct_aligned = m_ct.copy()
                # Because applying tf on arrow makes arrow coincide with x-axis of tf placement
                # but force is along z axis in local frame so need to transform x-->z , i.e. -90° around y
                m_ct_aligned.rotation = m_ct_aligned.rotation.dot(
                    pin.rpy.rpyToMatrix(0.0, -np.pi / 2, 0.0)
                )
                gui.applyConfiguration(
                    "world/force", list(pin.SE3ToXYZQUAT(m_ct_aligned))
                )
                # Move reference wrench
                m_ct_ref_aligned = m_ee_ref.copy()
                m_ct_ref_aligned.translation = m_ct_ref_aligned.act(
                    np.array([0.0, 0.0, offset])
                )
                m_ct_ref_aligned.rotation = m_ct_ref_aligned.rotation.dot(
                    pin.rpy.rpyToMatrix(0.0, -np.pi / 2, 0.0)
                )
            ...
            if "friction" in config["WHICH_COSTS"]:
                ...
                robot.viewer.gui.applyConfiguration(
                    "world/cone", list(np.array(pin.SE3ToXYZQUAT(position)).squeeze())
                )
