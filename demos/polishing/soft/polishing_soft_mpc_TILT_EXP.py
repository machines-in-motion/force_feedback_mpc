...
from force_feedback_mpc.core_mpc_utils import mpc_utils
...
def main(SAVE_DIR, TORQUE_TRACKING):
    ...
    for n_seed in range(N_SEEDS):
        ...
        for n_exp in range(N_EXP):
            ...
            u0 = pin_utils.get_tau(
                q0, v0, np.zeros(nq), f_ext0, robot.model, np.zeros(nq)
            )
            # Warmstart and solve
            ...
            contact_placement = pin.SE3(
                np.eye(3), np.asarray(config["contactPosition"])
            )
            ...
            contact_surface_bulletId = simulator_utils.display_contact_surface(
                contact_placement, bullet_endeff_ids=robot_simulator.bullet_endeff_ids
            )
            ...
            if config["DISPLAY_EE"]:
                ...
                for i in range(nb_points):
                    ...
                    ballsIdTarget[i] = simulator_utils.display_ball(
                        pos, RADIUS=0.01, COLOR=[1.0, 0.0, 0.0, 1.0]
                    )
                draw_rate = 1000
            ...
            ### INIT MPC SIMU ###
            # # # # # # # # # # #
            sim_data = MPCDataHandlerSoftContactAugmented(
                config, robot, softContactModel.nc
            )
            ...
            sensingModel = mpc_utils.SensorModel(
                config, naug=softContactModel.nc, SEED=SEEDS[n_seed]
            )
            if int(TORQUE_TRACKING) == 0:
                use = False
            else:
                use = True
            torqueController = mpc_utils.LowLevelTorqueController(
                config, nu=nu, use=use
            )
            ...
            for i in range(sim_data.N_simu):
                ...
                if time_to_contact == 0:
                    ...
                    position_at_contact_switch = robot.data.oMf[
                        id_endeff
                    ].translation.copy()
                ...
                if 0 <= time_to_contact and time_to_contact % OCP_TO_CTRL_RATIO == 0:
                    ...
                    target_force = target_force_traj[
                        time_to_contact:tf:OCP_TO_CTRL_RATIO, 2
                    ]
                ...
                if i >= T_CIRCLE:
                    ...
                    p_err.append(
                        np.abs(
                            robot_simulator.pin_robot.data.oMf[id_endeff].translation[
                                :2
                            ]
                            - target_position[0][:2]
                        )
                ...
                if config["DISPLAY_EE"] and i % draw_rate == 0:
                    pos = robot_simulator.pin_robot.data.oMf[
                        id_endeff
                    ].translation.copy()
                    ballId = simulator_utils.display_ball(
                        pos, RADIUS=0.03, COLOR=[0.0, 0.0, 1.0, 0.3]
                    )
