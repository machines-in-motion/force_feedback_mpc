...
def main(PLOT, DISPLAY):
    ...
    us_init = [
        pin_utils.get_tau(
            y0[:nq], y0[:nv], np.zeros(nv), fext0, robot.model, np.zeros(nv)
        )
    ...
    if PLOT:
        #  Plot
        ocp_data_handler = OCPDataHandlerSoftContactAugmented(
            solver.problem, softContactModel
        )
