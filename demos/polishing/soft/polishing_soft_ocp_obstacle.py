...
from force_feedback_mpc.core_mpc_utils import path_utils
...
if "1D" in config["contactType"]:
    softContactModel = SoftContactModel1D(
        np.asarray(config["Kp"]),
        np.asarray(config["Kv"]),
        oPc,
        id_endeff,
        config["contactType"],
        config["pinRefFrame"],
...
us_init = [
    pin_utils.get_tau(y0[:nq], y0[:nv], np.zeros(nv), fext0, robot.model, np.zeros(nv))
...
for k, m in enumerate(models):
    ...
    p_ee_ref = circle_point_WORLD(
        t, oMf, radius=RADIUS, omega=OMEGA, LOCAL_PLANE=config["CIRCLE_LOCAL_PLANE"]
    )
    # Cost translation
    ...
    xs_init.append(
        np.concatenate(
            [
                q_ws,
                v_ws,
                np.array([softContactModel.computeForce_(robot.model, q_ws, v_ws)]),
            ]
        )
    )
...
_, _ = ddp_handler.plot_ocp_results(
    ddp_data, which_plots=config["WHICH_PLOTS"], colors=["r"], markers=["."], SHOW=True
)
