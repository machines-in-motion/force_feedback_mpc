...
def main(SAVE_DIR, TORQUE_TRACKING):
    ...
    simulator_utils.set_contact_stiffness_and_damping(
        contact_surface_bulletId, 10000, 500
    )
    ...
    # Circle trajectory
    N_total_pos = int(
        (config["T_tot"] - config["T_REACH"]) / dt_simu
        + config["N_h"] * OCP_TO_CTRL_RATIO
    )
    N_circle = int(
        (config["T_tot"] - config["T_CIRCLE"]) / dt_simu
        + config["N_h"] * OCP_TO_CTRL_RATIO
    )
    ...
    sensingModel = mpc_utils.SensorModel(
        config, naug=softContactModel.nc, SEED=RANDOM_SEED
    )
