...
from force_feedback_mpc.core_mpc_utils import path_utils, mpc_utils
...
from force_feedback_mpc.soft_mpc.aug_ocp import (
    OptimalControlProblemSoftContactAugmented,
)
from force_feedback_mpc.soft_mpc.aug_data import (
...
# @profile
def solveOCP(
    q, v, f, solver, nb_iter, target_reach, anchor_point, TASK_PHASE, target_force
):
    t = time.time()
    ...
    if TASK_PHASE == 1:
        for k in range(solver.problem.T + 1):
            ...
            m[k].differential.costs.costs[
                "translation"
            ].cost.residual.reference = target_reach[k]
    ...
    if TASK_PHASE == 3:
        for k in range(solver.problem.T + 1):
            ...
            m[k].differential.costs.costs[
                "translation"
            ].cost.activation.weights = np.array([1.0, 1.0, 0.0])
            m[k].differential.costs.costs["translation"].weight = 50.0
            m[k].differential.costs.costs["rotation"].active = True
            m[k].differential.costs.costs[
                "rotation"
            ].cost.residual.reference = pin.utils.rpyToMatrix(np.pi, 0.0, np.pi)
    ...
    if TASK_PHASE == 4:
        for k in range(solver.problem.T + 1):
            ...
            m[k].differential.costs.costs["translation"].weight = 150.0
            m[k].differential.f_des = np.array([-target_force[k]])
            m[k].differential.costs.costs[
                "translation"
            ].cost.residual.reference = target_reach[k]
...
config = path_utils.load_yaml_file(
    os.path.dirname(os.path.realpath(__file__)) + "/" + config_name + ".yml"
)
# Create a simulation environment & simu-pin wrapper
dt_simu = 1.0 / float(config["simu_freq"])
q0 = np.asarray(config["q0"])
v0 = np.asarray(config["dq0"])
x0 = np.concatenate([q0, v0])
env = BulletEnvWithGround(dt=dt_simu, server=p.GUI)
robot_simulator = load_bullet_wrapper(
    "iiwa_convex_ft_sensor_shell", locked_joints=["A7"]
)
...
simulator_utils.display_ball(oPc, RADIUS=0.02, COLOR=[1.0, 0.0, 0.0, 0.2])
...
if config["TILT_SURFACE"]:
    TILT_RPY = [config["TILT_PITCH_LOCAL_DEG"] * np.pi / 180, 0.0, 0.0]
...
contact_surface_bulletId = simulator_utils.display_contact_surface(
    pin.SE3(np.eye(3), np.asarray(config["contactPosition"])),
...
# Contact model
softContactModel = SoftContactModel1D(
...
f_ext0 = [
    pin.Force.Zero() for _ in robot.model.joints
]  # pin_utils.get_external_joint_torques(contact_placement, f0, robot)
...
ocp = OptimalControlProblemSoftContactAugmented(robot, config).initialize(
    y0, softContactModel
)
...
for k, m in enumerate(models):
    ...
    for col_idx in range(len(robot.collision_model.collisionPairs)):
        ...
        m.differential.constraints.constraints[
            "collisionBox_" + str(col_idx)
        ].constraint.updateBounds(np.array([0.0]), np.array([np.inf]))
        # needed to pass the bounds to the manager
        m.differential.constraints.changeConstraintStatus(
            "collisionBox_" + str(col_idx), True
        )
...
N_total = int(
    (config["T_tot"] - config["T_CONTACT"]) / dt_simu
    + config["N_h"] * OCP_TO_CTRL_RATIO
)
N_ramp = int((config["T_RAMP"] - config["T_CONTACT"]) / dt_simu)

target_force_traj = np.zeros((N_total, 3))
target_force_traj[:N_ramp, 2] = [
    F_MIN + (F_MAX - F_MIN) * i / N_ramp for i in range(N_ramp)
]
...
# Circle trajectory
N_total_pos = int(
    (config["T_tot"] - config["T_REACH"]) / dt_simu + config["N_h"] * OCP_TO_CTRL_RATIO
)
N_circle = int(
    (config["T_tot"] - config["T_CIRCLE"]) / dt_simu + config["N_h"] * OCP_TO_CTRL_RATIO
)
target_position_traj = np.zeros((N_total_pos, 3))
# absolute desired position
target_position_traj[0:N_circle, :] = [
    np.array(
        [
            oPc[0] + RADIUS * (1 - np.cos(i * dt_simu * OMEGA)),
...
if config["DISPLAY_EE"]:
    ...
    for i in range(nb_points):
        ...
        simulator_utils.display_ball(pos, RADIUS=0.01, COLOR=[1.0, 0.0, 0.0, 1.0])
...
logger.debug(
    "OCP to PLAN ratio (# of re-replannings between two OCP nodes) = "
    + str(OCP_TO_MPC_RATIO)
)
logger.debug(
    "OCP to SIMU ratio (# of simulate steps between two OCP nodes) = "
    + str(OCP_TO_CTRL_RATIO)
)
...
for i in range(sim_data.N_simu):
    ...
    if 0 <= time_to_circle and time_to_circle % OCP_TO_CTRL_RATIO == 0:
        ...
        target_position[:, :2] = (
            target_position_traj[time_to_circle:tf:OCP_TO_CTRL_RATIO, :2]
            + position_at_contact_switch[:2]
            - oPc[:2]
        )
    ...
    if i % int(sim_data.simu_freq / sim_data.plan_freq) == 0:
        ...
        x_filtered = antiAliasingFilter.step(
            nb_plan, i, sim_data.plan_freq, sim_data.simu_freq, sim_data.state_mea_SIMU
        )
    ...
    if i % int(sim_data.simu_freq / sim_data.ctrl_freq) == 0:
        ...
        tau_mea_CTRL = antiAliasingFilter.step(
            nb_ctrl, i, sim_data.ctrl_freq, sim_data.simu_freq, sim_data.tau_mea_SIMU
        )
        tau_mea_derivative_CTRL = antiAliasingFilter.step(
            nb_ctrl,
            i,
            sim_data.ctrl_freq,
            sim_data.simu_freq,
        ...
        if config["RICCATI"]:
            y_filtered = antiAliasingFilter.step(
                nb_ctrl,
                i,
                sim_data.ctrl_freq,
                sim_data.simu_freq,
            ...
            tau_des_CTRL += solver.K[0].dot(
                solver.problem.x0 - y_filtered
            )  # position vel force

        if config["USE_LATERAL_FRICTION"] and 0 <= time_to_contact:
            Jac = pin.computeFrameJacobian(
                robot.model, robot.data, q, id_endeff, pin.LOCAL_WORLD_ALIGNED
            )[:3, :nq]
        ...
        tau_mot_CTRL = torqueController.step(
            tau_des_CTRL, tau_mea_CTRL, tau_mea_derivative_CTRL
        )
        # Increment control counter
        nb_ctrl += 1

    # Simulate actuation
    tau_mea_SIMU = actuationModel.step(
        tau_mot_CTRL, joint_vel=sim_data.state_mea_SIMU[i, nq : nq + nv]
    )
    ...
    if i >= T_CIRCLE:
        ...
        p_err.append(
            np.abs(
                robot_simulator.pin_robot.data.oMf[id_endeff].translation[:2]
                - target_position[0][:2]
            )
        )
    ...
    sim_data.record_simu_cycle_measured(
        i, y_mea_SIMU, y_mea_no_noise_SIMU, tau_mea_SIMU
    )

    # Display real
    if config["DISPLAY_EE"] and i % draw_rate == 0:
        pos = robot_simulator.pin_robot.data.oMf[id_endeff].translation.copy()
        simulator_utils.display_ball(pos, RADIUS=0.03, COLOR=[0.0, 0.0, 1.0, 0.3])
...
save_name = (
    config_name
    + "_BIAS="
    + str(config["SCALE_TORQUES"])
    + "_NOISE="
    + str(config["NOISE_STATE"] or config["NOISE_TORQUES"])
    + "_DELAY="
    + str(config["DELAY_OCP"] or config["DELAY_SIM"])
    + "_Fp="
    + str(sim_data.plan_freq / 1000)
    + "_Fc="
    + str(sim_data.ctrl_freq / 1000)
    + "_Fs"
    + str(sim_data.simu_freq / 1000)
)
# Extract plot data from sim data
plot_data = sim_data.extract_data(frame_of_interest=frame_of_interest)
# Plot results
sim_data.plot_mpc_results(
    plot_data,
    which_plots=sim_data.WHICH_PLOTS,
