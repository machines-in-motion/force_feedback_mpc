...
logger = CustomLogger(
    __name__, log_level_name=GLOBAL_LOG_LEVEL, USE_LONG_FORMAT=GLOBAL_LOG_FORMAT
).logger
...
for angle in TILT_ANGLES_DEG:
    TILT_RPY.append([0.0, angle * np.pi / 180, 0.0])
N_EXP = len(TILT_RPY)
# random seeds
# SEEDS   = [1, 2, 3]
SEEDS = [1, 2, 3, 4, 5]
...
prefix = (
    PREFIX
    + "iiwa_contact_circle_MPC_bullet__BIAS=True_NOISE=True_DELAY=True_Fp=0.5_Fc=0.5_Fs1.0"
)
...
def main(FILTER=1):
    ...
    for n_seed in range(N_SEEDS):
        ...
        for n_exp in range(N_EXP):
            ...
            data = data_utils.extract_plot_data_from_npz(
                prefix
                + "_EXP_TILT="
            ...
            Np = data["N_plan"]
            Ns = data["N_simu"]
            lin_pos_ee_ref = analysis_utils.linear_interpolation(
                data["lin_pos_ee_ref"], int((Ns + 1) / Np)
            )
            lin_err_ee_xy = np.zeros((data["lin_pos_ee_mea"].shape[0], 2))
            for i in range(lin_pos_ee_ref.shape[0]):
                lin_err_ee_xy[i, :] = np.abs(
                    data["lin_pos_ee_mea"][i, :2] - lin_pos_ee_ref[i, :2]
                )
            # Average absolute error
            p_avg_abs_err_xy_NORM[n_seed, n_exp] = np.linalg.norm(
                np.sum(lin_err_ee_xy, axis=0) / Ns
            )
            # Force tracking
            Np = data["N_plan"]
            Ns = data["N_simu"]
            f_ee_mea = data[
                "f_ee_mea"
            ]  # pin_utils.get_f_(data['q_mea'], data['v_mea'], data['u_des_SIMU'], data['pin_model'], data['pin_model'].getFrameId('contact') ) #
            f_ee_ref_z = -20
            ...
            bool_contact = np.isclose(
                f_ee_mea[:, 2], np.zeros(f_ee_mea[:, 2].shape), rtol=1e-3
            )
            cycles_not_in_contact[n_seed, n_exp] = (
            ...
            # Smooth if necessary
            if FILTER > 0:
                data["q_mea"] = analysis_utils.moving_average_filter(
                    data["q_mea"].copy(), FILTER
                )
                data["v_mea"] = analysis_utils.moving_average_filter(
                    data["v_mea"].copy(), FILTER
                )
                data["lin_pos_ee_mea"] = analysis_utils.moving_average_filter(
                    data["lin_pos_ee_mea"].copy(), FILTER
                )
                data["ang_pos_ee_mea"] = analysis_utils.moving_average_filter(
                    data["ang_pos_ee_mea"].copy(), FILTER
                )
                data["lin_vel_ee_mea"] = analysis_utils.moving_average_filter(
                    data["lin_vel_ee_mea"].copy(), FILTER
                )
                data["ang_vel_ee_mea"] = analysis_utils.moving_average_filter(
                    data["ang_vel_ee_mea"].copy(), FILTER
                )
            ...
            # Average absolute error
            p_avg_abs_err_xy_NORM_LPF[n_seed, n_exp] = np.linalg.norm(
                np.sum(lin_err_ee_xy, axis=0) / Ns
            )
            # Force tracking
            Np = data["N_plan"]
            ...
            f_ee_mea = data[
                "f_ee_mea"
            ]  # pin_utils.get_f_(data['q_mea'], data['v_mea'], data['tau_mea'], data['pin_model'], data['pin_model'].getFrameId('contact') )
            f_ee_err_z = np.zeros(f_ee_mea.shape[0])
            for i in range(Ns):
                f_ee_err_z[i] = np.abs(f_ee_mea[i, 2] - f_ee_ref_z)
            # Maximum (peak) absolute error along x,y,z
    ...
    ax2[0].plot(
        TILT_ANGLES_DEG,
        f_avg_abs_err_z_AVG_LPF,
        color=color_LPF,
        linestyle="-",
        linewidth=4,
        label="Force feedback MPC",
    )

    # FORCE MAX
    # Average
    ax2[1].plot(
        TILT_ANGLES_DEG, f_abs_max_z_AVG, color=color, linestyle="-", linewidth=4
    )
    ax2[1].plot(
        TILT_ANGLES_DEG,
        f_abs_max_z_AVG_LPF,
    ...
    for n_exp in range(N_EXP):
        ...
        ax1.plot(
            ...
            p_avg_abs_err_xy_NORM_AVG[n_exp],
            marker="o",
        ...
        ax1.plot(
            float(TILT_ANGLES_DEG[n_exp]),
            p_avg_abs_err_xy_NORM_AVG_LPF[n_exp],
            marker="s",
            markerfacecolor=color_LPF,
            markersize=18,
            markeredgecolor="k",
        )
        for n_seed in range(N_SEEDS):
            ax1.plot(
                ...
                p_avg_abs_err_xy_NORM[n_seed, n_exp],
                marker="o",
                markerfacecolor=color,
                markersize=16,
                alpha=0.3,
            ...
            ax1.plot(
                TILT_ANGLES_DEG[n_exp],
                p_avg_abs_err_xy_NORM_LPF[n_seed, n_exp],
                marker="s",
                markerfacecolor=color_LPF,
                markersize=16,
                alpha=0.3,
        ...
        for n_seed in range(N_SEEDS):
            ...
            ax2[0].plot(
                TILT_ANGLES_DEG[n_exp],
                f_avg_abs_err_z_LPF[n_seed, n_exp],
                marker="s",
                markerfacecolor=color_LPF,
                markersize=16,
                alpha=0.3,
        ...
        ax3.plot(
            float(TILT_ANGLES_DEG[n_exp]),
            cycles_not_in_contact_AVG[n_exp],
            marker="o",
            markerfacecolor=color,
            markersize=18,
            markeredgecolor="k",
        ...
        ax3.plot(
            float(TILT_ANGLES_DEG[n_exp]),
            cycles_not_in_contact_AVG_LPF[n_exp],
            marker="s",
            markerfacecolor=color_LPF,
            markersize=18,
            markeredgecolor="k",
