...
class MPCBenchmark:
    ...
    def record_profiles(self):
        for profile in self.profiles:
            ...
            self.avg[profile].append(
                1e3 * crocoddyl.stop_watch_get_average_time(profile)
            )
    ...
    def plot_profile(
        self, profile, SAVE=False, SAVE_DIR=None, SAVE_NAME=None, SHOW=True
    ):
        """
        Plot 1 profile
        Input:
        ...
        ax.fill_between(
            tspan, self.min[profile], self.max[profile], alpha=0.5, linewidth=0
        )
    ...
    def plot_avg_profiles(self, SAVE=False, SAVE_DIR=None, SAVE_NAME=None, SHOW=True):
        ...
        timings = [
            sum(self.avg[profile]) / len(self.avg[profile]) for profile in profiles
        ]
        fig, ax = plt.subplots(1, 1, figsize=(19.2, 10.8), sharex="col")
...
def linear_interpolation_demo():
    ...
    (input,) = plt.plot(
        np.linspace(0, 1, data.shape[0]), data[:, 1], "ro", label="input data"
    )
    (output,) = plt.plot(
        np.linspace(0, 1, interp.shape[0]), interp[:, 1], "g*", label="interpolated"
    )
