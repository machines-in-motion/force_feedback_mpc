...
def main(npz_path=None, FILTER=1, PLOT=False):
    ...
    if npz_path is None:
        ...
        for i in range(lin_pos_ee_ref.shape[0]):
            lin_err_ee_xyz[i, :] = np.abs(
                data["lin_pos_ee_mea"][i, :] - lin_pos_ee_ref[i, :]
            )
        ...
        print(
            " Average abs. EE xyz error norm : "
            + str(np.linalg.norm(lin_err_ee_xyz_avg))
        )
        print("\n")
        print("----------------------------------")
        # Force tracking
        ...
        bool_contact = np.isclose(
            data["f_ee_mea"][:, 2], np.zeros(data["f_ee_mea"][:, 2].shape), rtol=1e-6
        )
        cycles_not_in_contact = (100.0 * np.count_nonzero(bool_contact)) / Ns
