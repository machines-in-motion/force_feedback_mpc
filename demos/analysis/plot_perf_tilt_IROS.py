...
def main(FILTER=1, PLOT=False):
    ...
    for n_exp in range(N_EXP):
        ...
        data = data_utils.extract_plot_data_from_npz(
            prefix + "_EXP_TILT=" + str(TILT_ANGLES_DEG[n_exp]) + ".npz", LPF=False
        )
        ...
        data = data_utils.extract_plot_data_from_npz(
            prefix_lpf + "_EXP_TILT=" + str(TILT_ANGLES_DEG[n_exp]) + ".npz", LPF=True
        )
        Np = data["N_plan"]
        Ns = data["N_simu"]
        lin_pos_ee_ref = analysis_utils.linear_interpolation(
        ...
        lin_err_ee_xy_avg_LPF_NORM[n_exp] = np.linalg.norm(
            lin_err_ee_xy_avg_LPF[:, n_exp]
        )
        # Force tracking
        Np = data["N_plan"]
        Ns = data["N_simu"]
    ...
    fig2, ax2 = plt.subplots(
        2, 1, figsize=(19.2, 10.8)
    )  # Avg force err z + max force z
    xyz = ["x", "y", "z"]
    color_LPF = "r"
    color = "b"
    ...
    ax1.plot(
        TILT_ANGLES_DEG, lin_err_ee_xy_avg_NORM, color=color, linestyle="-", linewidth=3
    )
    ax1.plot(
        TILT_ANGLES_DEG,
        lin_err_ee_xy_avg_LPF_NORM,
        color=color_LPF,
        linestyle="-",
        linewidth=3,
    )
    ...
    ax2[1].plot(
        TILT_ANGLES_DEG, f_ee_err_max_z, color=color, linestyle="-", linewidth=3
    )
    ax2[1].plot(
        TILT_ANGLES_DEG, f_ee_err_max_z_LPF, color=color_LPF, linestyle="-", linewidth=3
    )
    ...
    for n_exp in range(N_EXP):
        ...
        ax1.plot(
            float(TILT_ANGLES_DEG[n_exp]),
            lin_err_ee_xy_avg_LPF_NORM[n_exp],
            marker="s",
