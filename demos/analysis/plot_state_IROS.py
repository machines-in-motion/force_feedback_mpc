...
def main(LPF=False, FILTER=0):
    ...
    if LPF:
        ...
        plot_data = data_utils.extract_plot_data_from_npz(
            PREFIX + data_file_name_lpf + ".npz", LPF=True
        )
    else:
        print("Opening " + data_file_name + "...")
        plot_data = data_utils.extract_plot_data_from_npz(
            PREFIX + data_file_name + ".npz", LPF=False
        )

    # Smooth if necessary
    print("Filtering signals...")
    if FILTER > 0:
        plot_data["lin_pos_ee_mea"] = analysis_utils.moving_average_filter(
            plot_data["lin_pos_ee_mea"].copy(), FILTER
        )
        plot_data["f_ee_mea"] = analysis_utils.moving_average_filter(
            plot_data["f_ee_mea"].copy(), FILTER
        )
    # Plot
    ...
    for i in range(nq):
        if PLOT_PREDICTIONS:
            ...
            for j in range(0, N_plan, pred_plot_sampling):
                ...
                points_tau = (
                    np.array([tspan_x_pred, tau_pred_i[j, :]])
                    .transpose()
                    .reshape(-1, 1, 2)
                )
    ...
    # y axis labels
    fig.text(
        0.06, 0.5, "Joint position (rad)", va="center", rotation="vertical", fontsize=12
    )
    fig.text(
        0.345,
        0.5,
    ...
    fig.text(
        0.625, 0.5, "Joint torque (Nm)", va="center", rotation="vertical", fontsize=12
    )
