...
prefix_lpf = (
    PREFIX
    + "iiwa_LPF_sanding_MPC_bullet__BIAS=True_NOISE=True_DELAY=True_Fp=0.5_Fc=1.0_Fs2.0"
)
prefix_soft = (
    PREFIX
    + "iiwa_aug_soft_sanding_MPC_bullet__BIAS=True_NOISE=True_DELAY=True_Fp=0.5_Fc=1.0_Fs2.0"
)
prefix_classical = (
    PREFIX
    + "iiwa_sanding_MPC_bullet__BIAS=True_NOISE=True_DELAY=True_Fp=0.5_Fc=1.0_Fs2.0"
)

CUTOFF = 3.0  # in seconds

# tilt table of several angles around y-axis
TILT_ANGLES_DEG = [6, 4, 2, 0, -2, -4, -6]
TILT_RPY = []
for angle in TILT_ANGLES_DEG:
    TILT_RPY.append([angle * np.pi / 180, 0.0, 0.0])
N_EXP = len(TILT_RPY)
...
for n_seed in range(N_SEEDS):
    ...
    for n_exp in range(N_EXP):
        ...
        data = sd
        # Interpolate
        tau_des = linear_interpolation(
            data["u_des_PLAN"], int(data["N_simu"] / data["N_plan"])
        )
        # Smooth if necessary
        if FILTER > 0:
            data["lin_pos_ee_mea"] = analysis_utils.moving_average_filter(
                data["lin_pos_ee_mea"].copy(), FILTER
            )
            data["f_ee_mea"] = analysis_utils.moving_average_filter(
                data["f_ee_mea"].copy(), FILTER
            )
            tau_mea = analysis_utils.moving_average_filter(
                data["tau_mea_SIMU"].copy(), FILTER
            )
        ...
        for i in range(Ns - 4):
            tau_err.append(
                np.abs(tau_mea[i + N_START_SIMU, :] - tau_des[i + N_START_SIMU, :])
            )
        ...
        for i in range(Ns):
            position_error.append(
                np.abs(
                    data["lin_pos_ee_mea"][i + N_START_SIMU, :2]
                    - data["lin_pos_ee_ref"][int(i * Np / Ns) + N_START_PLAN, :2]
                )
            )
        ...
        for i in range(Ns):
            force_error.append(
                np.abs(data["f_ee_mea"][i + N_START_SIMU, 2] - force_reference)
            )
        force_error_MAX_classical[n_seed, n_exp] = np.max(
            np.abs(data["f_ee_mea"][N_START_SIMU:, 2])
        )
        force_error_LAT_classical[n_seed, n_exp] = np.mean(
            np.abs(data["f_ee_mea"][N_START_SIMU:, :2])
        )
        force_error_AVG_classical[n_seed, n_exp] = np.mean(force_error)
        # Is in contact
        bool_contact = np.isclose(
            data["f_ee_mea"][N_START_SIMU:, 2],
            np.zeros(data["f_ee_mea"][N_START_SIMU:, 2].shape),
            rtol=1e-3,
        )
        cycles_not_in_contact_classical[n_seed, n_exp] = (
            100.0 * np.count_nonzero(bool_contact)
        ) / Ns

        logger.warning(
            "Classical MPC avg torque error    = "
            + str(torque_error_AVG_NORM_classical[n_seed, n_exp])
        )
        logger.warning(
            "Classical MPC avg position error  = "
            + str(position_error_AVG_NORM_classical[n_seed, n_exp])
        )
        logger.warning(
            "Classical MPC avg force error     = "
            + str(force_error_AVG_classical[n_seed, n_exp])
        )
        logger.warning(
            "Classical MPC max force           = "
            + str(force_error_MAX_classical[n_seed, n_exp])
        )
        logger.warning(
            "Classical MPC not-in-contact rate = "
            + str(cycles_not_in_contact_classical[n_seed, n_exp])
        )
        logger.warning(
            "Classical MPC LATERAL FORCES NORM = "
            + str(force_error_LAT_classical[n_seed, n_exp])
        )
        ...
        # Interpolate
        tau_des = linear_interpolation(
            data["tau_des_PLAN"], int(data["N_simu"] / data["N_plan"])
        )
        # Smooth if necessary
        if FILTER > 0:
            data["lin_pos_ee_mea"] = analysis_utils.moving_average_filter(
            ...
            data["tau_mea"] = analysis_utils.moving_average_filter(
                data["tau_mea"].copy(), FILTER
            )
            tau_des = analysis_utils.moving_average_filter(tau_des.copy(), FILTER)
        # Compute MAE |mea - ref|
        N_START_SIMU = int(CUTOFF * data["simu_freq"])
        ...
        for i in range(Ns):
            tau_err.append(
                np.abs(
                    data["tau_mea"][i + N_START_SIMU, :] - tau_des[i + N_START_SIMU, :]
                )
            )
        ...
        force_error_MAX_lpf[n_seed, n_exp] = np.max(
            np.abs(data["f_ee_mea"][N_START_SIMU:, 2])
        )
        force_error_LAT_lpf[n_seed, n_exp] = np.mean(
            np.abs(data["f_ee_mea"][N_START_SIMU:, :2])
        )
        ...
        cycles_not_in_contact_lpf[n_seed, n_exp] = (
            100.0 * np.count_nonzero(bool_contact)
        ) / Ns

        logger.warning(
            "LPF MPC avg torque error    = "
            + str(torque_error_AVG_NORM_lpf[n_seed, n_exp])
        )
        logger.warning(
            "LPF MPC avg position error  = "
            + str(position_error_AVG_NORM_lpf[n_seed, n_exp])
        )
        logger.warning(
            "LPF MPC avg force error     = " + str(force_error_AVG_lpf[n_seed, n_exp])
        )
        logger.warning(
            "LPF MPC max force           = " + str(force_error_MAX_lpf[n_seed, n_exp])
        )
        logger.warning(
            "LPF MPC not-in-contact rate = "
            + str(cycles_not_in_contact_lpf[n_seed, n_exp])
        )
        logger.warning(
            "LPF MPC LATERAL FORCES NORM = " + str(force_error_LAT_lpf[n_seed, n_exp])
        )
        ...
        for i in range(Ns):
            force_error.append(
                np.abs(data["f_ee_mea"][i + N_START_SIMU] - force_reference)
            )
        force_error_MAX_soft[n_seed, n_exp] = np.max(
            np.abs(data["f_ee_mea"][N_START_SIMU:])
        )
        force_error_AVG_soft[n_seed, n_exp] = np.sum(force_error, axis=0) / Ns
        # Is in contact
        bool_contact = np.isclose(
            data["f_ee_mea"][N_START_SIMU:],
            np.zeros(data["f_ee_mea"][N_START_SIMU:].shape),
            rtol=1e-3,
        )
        cycles_not_in_contact_soft[n_seed, n_exp] = (
            100.0 * np.count_nonzero(bool_contact)
        ) / Ns

        logger.warning(
            "Soft MPC avg torque error    = "
            + str(torque_error_AVG_NORM_soft[n_seed, n_exp])
        )
        logger.warning(
            "Soft MPC avg position error  = "
            + str(position_error_AVG_NORM_soft[n_seed, n_exp])
        )
        logger.warning(
            "Soft MPC avg force error     = " + str(force_error_AVG_soft[n_seed, n_exp])
        )
        logger.warning(
            "Soft MPC max force           = " + str(force_error_MAX_soft[n_seed, n_exp])
        )
        logger.warning(
            "Soft MPC not-in-contact rate = "
            + str(cycles_not_in_contact_soft[n_seed, n_exp])
        )
...
torque_error_AVG_NORM_classical_AVG = (
    np.sum(torque_error_AVG_NORM_classical, axis=0) / N_SEEDS
)
...
position_error_AVG_NORM_classical_AVG = (
    np.sum(position_error_AVG_NORM_classical, axis=0) / N_SEEDS
)
position_error_AVG_NORM_lpf_AVG = np.sum(position_error_AVG_NORM_lpf, axis=0) / N_SEEDS
position_error_AVG_NORM_soft_AVG = (
    np.sum(position_error_AVG_NORM_soft, axis=0) / N_SEEDS
)
...
cycles_not_in_contact_classical_AVG = (
    np.sum(cycles_not_in_contact_classical, axis=0) / N_SEEDS
)
...
torque_errors_AVG = [
    torque_error_AVG_NORM_classical_AVG,
    torque_error_AVG_NORM_lpf_AVG,
    torque_error_AVG_NORM_soft_AVG,
]
position_errors_AVG = [
    position_error_AVG_NORM_classical_AVG,
    position_error_AVG_NORM_lpf_AVG,
    position_error_AVG_NORM_soft_AVG,
]
force_errors_AVG_AVG = [
    force_error_AVG_classical_AVG,
    force_error_AVG_lpf_AVG,
    force_error_AVG_soft_AVG,
]
force_errors_MAX_AVG = [
    force_error_MAX_classical_AVG,
    force_error_MAX_lpf_AVG,
    force_error_MAX_soft_AVG,
]
cycles_not_in_contact_AVG = [
    cycles_not_in_contact_classical_AVG,
    cycles_not_in_contact_lpf_AVG,
    cycles_not_in_contact_soft_AVG,
]

torque_errors = [
    torque_error_AVG_NORM_classical,
    torque_error_AVG_NORM_lpf,
    torque_error_AVG_NORM_soft,
]
position_errors = [
    position_error_AVG_NORM_classical,
    position_error_AVG_NORM_lpf,
    position_error_AVG_NORM_soft,
]
force_errors_AVG = [
    force_error_AVG_classical,
    force_error_AVG_lpf,
    force_error_AVG_soft,
]
force_errors_MAX = [
    force_error_MAX_classical,
    force_error_MAX_lpf,
    force_error_MAX_soft,
]
cycles_not_in_contact = [
    cycles_not_in_contact_classical,
    cycles_not_in_contact_lpf,
    cycles_not_in_contact_soft,
]
...
error_dict["position_errors"] = (
    position_errors  # MAEs in that order : classical, lpf, soft
)
error_dict["force_errors_AVG"] = (
    force_errors_AVG  # MAEs in that order : classical, lpf, soft
)
error_dict["force_errors_MAX"] = (
    force_errors_MAX  # MAEs in that order : classical, lpf, soft
)
error_dict["cycles_not_in_contact"] = (
    cycles_not_in_contact  # MAEs in that order : classical, lpf, soft
)
...
for i in range(3):
    ...
    for n_exp in range(N_EXP):
        ax0.plot(
            float(TILT_ANGLES_DEG[n_exp]),
            torque_errors_AVG[i][n_exp],
            marker=markers[i],
        ...
        ax3.plot(
            float(TILT_ANGLES_DEG[n_exp]),
            cycles_not_in_contact_AVG[i][n_exp],
            marker=markers[i],
            markerfacecolor=colors[i],
            markersize=18,
            markeredgecolor="k",
        ...
        for n_seed in range(N_SEEDS):
            ax0.plot(
                TILT_ANGLES_DEG[n_exp],
                torque_errors[i][n_seed, n_exp],
                marker=markers[i],
                markerfacecolor=colors[i],
                markersize=16,
                alpha=0.3,
            ...
            ax1.plot(
                TILT_ANGLES_DEG[n_exp],
                position_errors[i][n_seed, n_exp],
                marker=markers[i],
                markerfacecolor=colors[i],
                markersize=16,
                alpha=0.3,
            ...
            ax2[0].plot(
                TILT_ANGLES_DEG[n_exp],
                force_errors_AVG[i][n_seed, n_exp],
                marker=markers[i],
                markerfacecolor=colors[i],
                markersize=16,
                alpha=0.3,
            ...
            ax2[1].plot(
                TILT_ANGLES_DEG[n_exp],
                force_errors_MAX[i][n_seed, n_exp],
                marker=markers[i],
                markerfacecolor=colors[i],
                markersize=16,
                alpha=0.3,
            ...
            ax3.plot(
                TILT_ANGLES_DEG[n_exp],
                cycles_not_in_contact[i][n_seed, n_exp],
                marker=markers[i],
                markerfacecolor=colors[i],
                markersize=16,
                alpha=0.3,
