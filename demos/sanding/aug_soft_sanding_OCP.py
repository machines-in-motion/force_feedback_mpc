...
config = load_yaml_file('/home/skleff/misc_repos/force_feedback_dgh/config/polishing_soft.yml')
...
RADIUS = 0.07 # config['frameCircleTrajectoryRadius'] 
OMEGA  = 3. #config['frameCircleTrajectoryVelocity']
for k,m in enumerate(models):
    # Ref
    ...
    p_ee_ref = circle_point_WORLD(t, oMf, 
                                            radius=RADIUS,
                                            omega=OMEGA,
                                            LOCAL_PLANE='XY') #config['CIRCLE_LOCAL_PLANE'])
    # Cost translation
...
if(PLOT):
    ...
    _, _ = ddp_handler.plot_ocp_results(ddp_data, which_plots=['all'], 
                                                        colors=['r'], 
                                                        markers=['.'], 
                                                        SHOW=True)
