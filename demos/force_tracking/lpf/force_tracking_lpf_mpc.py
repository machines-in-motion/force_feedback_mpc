...
def main(robot_name="iiwa", simulator="bullet", PLOT_INIT=False):
    ...
    if simulator == "bullet":
        ...
        env, robot_simulator, _ = simulator_utils.init_bullet_simulation(
            robot_name, dt=dt_simu, x0=x0
        )
        ...
        env, robot_simulator = simulator_utils.init_raisim_simulation(
            robot_name, dt=dt_simu, x0=x0
        )
    ...
    simulator_utils.display_ball(
        contactTranslationTarget, RADIUS=0.02, COLOR=[1.0, 0.0, 0.0, 0.2]
    )
    # Display contact surface
    import pinocchio as pin

    contact_placement = pin.SE3(np.eye(3), np.asarray(contactPosition))
    contactId = simulator_utils.display_contact_surface(
    ...
    u0 = pin_utils.get_u_grav(
        q0, robot.model, config["armature"]
    )  # pin_utils.get_tau(q0, v0, np.zeros((nq,1)), f_ext, robot.model, config['armature'])
    ...
    ddp = OptimalControlProblemLPF(robot, config, lpf_joint_names).initialize(
        y0, callbacks=False
    )
    ...
    if PLOT_INIT:
        ...
        ddp_data = ddp_handler.extract_data(
            ee_frame_name=frame_of_interest, ct_frame_name=frame_of_interest
        )
        _, _ = ddp_handler.plot_ddp_results(
            ddp_data,
            which_plots=config["WHICH_PLOTS"],
            colors=["r"],
            markers=["."],
            SHOW=True,
        )
    ...
    OCP_TO_MPC_CYCLES = 1.0 / (sim_data.dt_plan / config["dt"])
    OCP_TO_SIMU_CYCLES = 1.0 / (sim_data.dt_simu / config["dt"])
    ...
    for i in range(config["N_simu"]):
        ...
        if i >= T_REACH and i < T_REACH + NH_SIMU:
            ...
            if int(T_REACH - i) % OCP_TO_SIMU_CYCLES == 0:
                ...
                if int(T_REACH - i) == 0:
                    ...
                    ddp.problem.terminalModel.differential.costs.costs[
                        "translation"
                    ].cost.residual.reference = contactTranslationTarget
                else:
                    node_idx = config["N_h"] + int((T_REACH - i) / OCP_TO_SIMU_CYCLES)
                    ddp.problem.runningModels[node_idx].differential.costs.costs[
                        "translation"
                    ].active = True
                    ddp.problem.runningModels[node_idx].differential.costs.costs[
                        "translation"
        ...
        if i >= T_CONTACT and i <= T_CONTACT + NH_SIMU:
            ...
            if int(T_CONTACT - i) % OCP_TO_SIMU_CYCLES == 0:
                ...
                if int(T_CONTACT - i) == 0:
                    ...
                    ddp.problem.terminalModel.differential.contacts.contacts[
                        "contact"
                    ].contact.active = True
                    ...
                    ddp.problem.runningModels[node_idx].differential.contacts.contacts[
                        "contact"
                    ].contact.active = True
                    ddp.problem.runningModels[node_idx].differential.costs.costs[
                        "force"
                    ...
                    ddp.problem.runningModels[node_idx].differential.costs.costs[
                        "force"
                    ].cost.weight = MAX_FORCE_WEIGHT  # weight #
                    ddp.problem.runningModels[node_idx].differential.costs.costs[
                        "stateReg"
                    ].cost.residual.reference = np.concatenate([qref, np.zeros(nv)])
                    logger.debug(
                        ddp.problem.runningModels[node_idx]
                        .differential.contacts.contacts["contact"]
                        .contact.active
                    )
                    # logger.debug(ddp.problem.runningModels[node_idx].differential.contacts.active)
        if i == T_CONTACT + NH_SIMU:
            logger.debug("Fully inside the CONTACT phase")
        ...
        tau_mea_SIMU = actuationModel.step(
            i, tau_ref_SIMU, sim_data.state_mea_SIMU[:, -nu:]
        )
        ...
        if config["RICCATI"]:
            ...
            ) * np.eye(nq)  # only for torques
            tau_mea_SIMU += Ktilde[:, : nq + nv].dot(
                ddp.problem.x0[: nq + nv] - sim_data.state_mea_SIMU[i, : nq + nv]
            )  # position vel
            tau_mea_SIMU += Ktilde[:, :-nq].dot(
                ddp.problem.x0[:-nq] - sim_data.state_mea_SIMU[i, :-nq]
            )  # torques
        ...
        f_mea_SIMU = simulator_utils.get_contact_wrench(
            robot_simulator, id_endeff, sim_data.PIN_REF_FRAME
        )
        if i % 50 == 0:
        ...
        sim_data.state_mea_SIMU[i + 1, :] = sensingModel.step(
            i, y_mea_SIMU, sim_data.state_mea_SIMU
        )
