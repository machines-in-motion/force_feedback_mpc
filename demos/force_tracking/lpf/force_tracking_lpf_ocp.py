...
for k, m in enumerate(models):
    m.differential.costs.costs["translation"].cost.residual.reference = np.array(
        [0.65, 0.0, -0.01]
    )
for k, m in enumerate(models[:-1]):
    m.differential.costs.costs["force"].active = True
    m.differential.costs.costs["force"].cost.residual.reference = pin.Force(
        np.array([0.0, 0.0, 50, 0.0, 0.0, 0.0])
    )
...
if PLOT:
    #  Plot
    ocp_data_handler = OCPDataHandlerLPF(
        solver.problem, n_lpf=len(robot.model.names[1:])
    )
    ocp_data = ocp_data_handler.extract_data(solver.xs, solver.us)
    _, _ = ocp_data_handler.plot_ocp_results(
        ocp_data, which_plots=["all"], colors=["r"], markers=["."], SHOW=True
    )
