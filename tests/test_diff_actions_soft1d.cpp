...
void test_check_data(DAMSoftContact1DTypes::Type action_type,
  ...
  std::shared_ptr<
      ...
      model = factory.create(action_type, ref_type, mask_type);
...
void test_attributes(DAMSoftContact1DTypes::Type action_type,
  ...
  // Force cost
  BOOST_CHECK((model->get_force_weight() - Eigen::VectorXd::Ones(1) * 0.01)
                  .isZero(NUMDIFF_MODIFIER * tol));
  ...
  Eigen::VectorXd force_rate_reg_weight =
      rand() % 100 * Eigen::VectorXd::Ones(1);
...
void test_partials_numdiff(
  ...
  // std::cout << "With force cost = " << model->get_with_force_cost() <<
  // std::endl; std::cout << "With force rate reg cost = " <<
  // model->get_with_force_rate_reg_cost() << std::endl; std::cout << "With
  // gravity torque reg cost = " << model->get_with_gravity_torque_reg() <<
  // std::endl; Computing the action derivatives
  model->calc(data, x, f, u);
  std::shared_ptr<
      force_feedback_mpc::softcontact::DADSoftContact1DAugmentedFwdDynamics>
      data_cast =
          std::static_pointer_cast<force_feedback_mpc::softcontact::
                                       DADSoftContact1DAugmentedFwdDynamics>(
              data);
  ...
  const Eigen::VectorXd& fn0 =
      std::static_pointer_cast<force_feedback_mpc::softcontact::
                                   DADSoftContact1DAugmentedFwdDynamics>(data)
          ->fout;
...
void test_partial_derivatives_against_numdiff(
    DAMSoftContact1DTypes::Type action_type, pinocchio::ReferenceFrame ref_type,
    Vector3MaskType mask_type) {
  // create the model
  DAMSoftContact1DFactory factory;
  std::shared_ptr<
...
void test_calc_equivalent_free(DAMSoftContact1DTypes::Type action_type,
  ...
  std::shared_ptr<
      force_feedback_mpc::softcontact::DAMSoftContact1DAugmentedFwdDynamics>
      modelsoft = factory.create(action_type, ref_type, mask_type);
  ...
  std::shared_ptr<
      force_feedback_mpc::softcontact::DAMSoftContact1DAugmentedFwdDynamics>
      modelsoft2 = factory.create(action_type, ref_type, mask_type);
...
void register_action_model_unit_tests(
  ...
  ts->add(BOOST_TEST_CASE(boost::bind(&test_calc_returns_a_cost, action_type,
                                      ref_type, mask_type)));
  ts->add(BOOST_TEST_CASE(
      boost::bind(&test_attributes, action_type, ref_type, mask_type)));
