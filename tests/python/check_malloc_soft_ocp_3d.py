                        ...
                        for with_force_rate_reg_cost in [True, False]:
                            if VERBOSE:
                                print(
                                    ...
                                    + str(with_armature)
                                    + ", grav_cost = "
                                    + str(with_gravity_torque_reg)
                                    + ", force_cost = "
                                    + str(with_force_cost)
                                    + ", force_rate_cost = "
                                    + str(with_force_rate_reg_cost)
                                )
                            # Set attributes
                            ...
                            m.differential.with_gravity_torque_reg = (
                                with_gravity_torque_reg
                            )
                            m.differential.with_force_cost = with_force_cost
                            m.differential.with_force_rate_reg_cost = (
                                with_force_rate_reg_cost
                            )
                            # Check calc
                            # print("        > Check DAM.calc")
                            m.differential.calc(
                                d.differential, x0, f0[-softContactModel.nc :], u0
                            )
                            # print("        > Check DAM.calcDiff")
                            m.differential.calcDiff(
                                d.differential, x0, f0[-softContactModel.nc :], u0
                            )
                            for with_force_constraint in [True, False]:
                                for with_friction_cone_constraint in [True, False]:
                                    if VERBOSE:
                                        print(
                                            "force_cstr    = "
                                            + str(with_force_constraint)
                                        )
                                        print(
                                            "friction_cstr = "
                                            + str(with_friction_cone_constraint)
                                        )
                                    m.with_force_constraint = with_force_constraint
                                    m.friction_constraints[
                                        0
                                    ].active = with_friction_cone_constraint
