...
const std::vector<DAMSoftContact3DFrictionTypes::Type>
    DAMSoftContact3DFrictionTypes::all(
        DAMSoftContact3DFrictionTypes::init_all());
...
DAMSoftContact3DFrictionFactory::create(
    DAMSoftContact3DFrictionTypes::Type dam_type,
  ...
  switch (dam_type) {
    ...
    default:
      throw_pretty(__FILE__
                   ": Wrong DAMSoftContact3DFrictionTypes::Type given");
      break;
  }
  return action;
}
...
std::shared_ptr<sobec::DAMSoftContact3DAugmentedFrictionFwdDynamics>
DAMSoftContact3DFrictionFactory::create_augmentedDAMSoft3DFriction(
  ...
  action =
      std::make_shared<sobec::DAMSoftContact3DAugmentedFrictionFwdDynamics>(
          state, actuation, cost, state->get_pinocchio()->getFrameId(frameName),
