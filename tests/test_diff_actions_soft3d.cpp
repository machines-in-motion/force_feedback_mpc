...
void test_check_data(DAMSoftContact3DTypes::Type action_type,
  ...
  std::shared_ptr<
      ...
      model = factory.create(action_type, ref_type);
...
void test_attributes(DAMSoftContact3DTypes::Type action_type,
  ...
  // Contact model
  BOOST_CHECK((model->get_Kp() - Eigen::VectorXd::Ones(model->get_nc()) * 100)
                  .isZero(NUMDIFF_MODIFIER * tol));
  BOOST_CHECK((model->get_Kv() - Eigen::VectorXd::Ones(model->get_nc()) * 10)
                  .isZero(NUMDIFF_MODIFIER * tol));
  ...
  // Force cost
  BOOST_CHECK((model->get_force_weight() - Eigen::Vector3d::Ones())
                  .isZero(NUMDIFF_MODIFIER * tol));
  BOOST_CHECK(model->get_force_des().isZero(NUMDIFF_MODIFIER * tol));
  BOOST_CHECK(model->get_with_force_cost());
  BOOST_CHECK(model->get_cost_ref() ==
              pinocchio::ReferenceFrame::LOCAL_WORLD_ALIGNED);
  // Force rate cost
  BOOST_CHECK(
      (model->get_force_rate_reg_weight() - 1e-6 * Eigen::Vector3d::Ones())
          .isZero(NUMDIFF_MODIFIER * tol));
  ...
  BOOST_CHECK(
      (model->get_force_des() - force_des).isZero(NUMDIFF_MODIFIER * tol));

  Eigen::Vector3d force_rate_reg_weight =
      rand() % 100 * Eigen::Vector3d::Ones();
  model->set_force_rate_reg_weight(force_rate_reg_weight);
  BOOST_CHECK((model->get_force_rate_reg_weight() - force_rate_reg_weight)
                  .isZero(NUMDIFF_MODIFIER * tol));
  ...
  BOOST_CHECK(
      (model->get_armature() - armature).isZero(NUMDIFF_MODIFIER * tol));
}
...
void test_partials_numdiff(
  ...
  // if(model->get_with_gravity_torque_reg() == true ||
  // model->get_with_force_rate_reg_cost() == true){
  //   with_gauss_approx = true;
  // }
  // std::cout << " Gravity cost      = " <<
  // model->get_with_gravity_torque_reg() << std::endl; std::cout << " Force
  // cost        = " << model->get_with_force_cost() << std::endl; std::cout <<
  // " Force rate cost   = " << model->get_with_force_rate_reg_cost() <<
  // std::endl; std::cout << " Gauss-Newton      = " << with_gauss_approx <<
  // std::endl;
  model->calc(data, x, f, u);
  std::shared_ptr<
      force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>
      data_cast =
          std::static_pointer_cast<force_feedback_mpc::softcontact::
                                       DADSoftContact3DAugmentedFwdDynamics>(
              data);
  // std::cout << " Residual gravity    = " << data_cast->tau_grav_residual <<
  // std::endl; std::cout << " Residual force      = " << data_cast->f_residual
  // << std::endl; std::cout << " Residual total      = " << data_cast->residual
  // << std::endl;
  ...
  const Eigen::VectorXd& fn0 =
      std::static_pointer_cast<force_feedback_mpc::softcontact::
                                   DADSoftContact3DAugmentedFwdDynamics>(data)
          ->fout;
  ...
  std::shared_ptr<
      ...
      data_num_diff_cast =
          std::static_pointer_cast<force_feedback_mpc::softcontact::
                                       DADSoftContact3DAugmentedFwdDynamics>(
              data_num_diff);
  std::vector<std::shared_ptr<crocoddyl::DifferentialActionDataAbstract>>
      data_x;
  std::vector<std::shared_ptr<crocoddyl::DifferentialActionDataAbstract>>
      data_f;
  std::vector<std::shared_ptr<crocoddyl::DifferentialActionDataAbstract>>
  ...
  //     model->get_state()->integrate(x, dx, xp);
  //     model->calc(data_x[ix], xp, f, u);
  //     std::shared_ptr<force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>
  //     data_ix_cast =
  //     std::static_pointer_cast<force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>(data_x[ix]);
  ...
  //       model->get_state()->integrate(x, dx, xp);
  //       model->calc(data_x[ix], xp, f, u);
  //       const double cpp = data_ix_cast->cost;  // cost due to positive
  //       disturbance in both directions dx(ix) = 0.;
  //       model->get_state()->integrate(x, dx, xp);
  //       model->calc(data_x[ix], xp, f, u);
  //       const double czp = data_ix_cast->cost;  // cost due to zero
  //       disturance in 'i' and positive disturbance in 'j' direction
  //       data_num_diff_cast->Lxx(ix, jx) = (cpp - czp - cp + c0) /
  //       xh_hess_pow2; data_num_diff_cast->Lxx(jx, ix) =
  //       data_num_diff_cast->Lxx(ix, jx); dx(ix) = xh_hess; dx(jx) = 0.;
  //     }
  //     dx(ix) = 0.;
  //   }
  ...
  //     std::shared_ptr<force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>
  //     data_idf_cast =
  //     std::static_pointer_cast<force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>(data_f[idf]);
  ...
  //       df(jdf) = fh_hess;
  //       model->calc(data_f[idf], x, f + df, u);
  //       const double cpp = data_idf_cast->cost;  // cost due to positive
  //       disturbance in both directions df(idf) = 0.; model->calc(data_f[idf],
  //       x, f + df, u); const double czp = data_idf_cast->cost;  // cost due
  //       to zero disturance in 'i' and positive disturbance in 'j' direction
  //       data_num_diff_cast->Lff(idf, jdf) = (cpp - czp - cp + c0) /
  //       fh_hess_pow2; data_num_diff_cast->Lff(jdf, idf) =
  //       data_num_diff_cast->Lff(idf, jdf); df(idf) = fh_hess; df(jdf) = 0.;
  //     }
  //     df(idf) = 0.;
  //   }
  ...
  //     std::shared_ptr<force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>
  //     data_iu_cast =
  //     std::static_pointer_cast<force_feedback_mpc::softcontact::DADSoftContact3DAugmentedFwdDynamics>(data_u[iu]);
  ...
  //       du(ju) = uh_hess;
  //       model->calc(data_u[iu], x, f, u + du);
  //       const double cpp = data_iu_cast->cost;  // cost due to positive
  //       disturbance in both directions du(iu) = 0.; model->calc(data_u[iu],
  //       x, f, u + du); const double czp = data_iu_cast->cost;  // cost due to
  //       zero disturance in 'i' and positive disturbance in 'j' direction
  //       data_num_diff_cast->Luu(iu, ju) = (cpp - czp - cp + c0) /
  //       uh_hess_pow2; data_num_diff_cast->Luu(ju, iu) =
  //       data_num_diff_cast->Luu(iu, ju); du(iu) = uh_hess; du(ju) = 0.;
  //     }
  //     du(iu) = 0.;
  //   }
  ...
  // if(!with_gauss_approx){
  //   BOOST_CHECK((datacast->Lxx -
  //   data_num_diff_cast->Lxx).isZero(NUMDIFF_MODIFIER * tol));
  //   BOOST_CHECK((datacast->Lff -
  //   data_num_diff_cast->Lff).isZero(NUMDIFF_MODIFIER * tol));
  //   BOOST_CHECK((datacast->Luu -
  //   data_num_diff_cast->Luu).isZero(NUMDIFF_MODIFIER * tol));
  //   BOOST_CHECK((datacast->Lxu -
  //   data_num_diff_cast->Lxu).isZero(NUMDIFF_MODIFIER * tol));
  // }
}
...
void test_partial_derivatives_against_numdiff_armature(
  ...
  std::shared_ptr<
      ...
      model = factory.create(action_type, ref_type);
  Eigen::VectorXd armature =
      1e-3 * Eigen::VectorXd::Ones(model->get_state()->get_nv());
  // optional armature
...
void test_calc_free(
  ...
  std::shared_ptr<crocoddyl::DifferentialActionModelFreeFwdDynamics> modelfree =
      std::make_shared<crocoddyl::DifferentialActionModelFreeFwdDynamics>(
          statemb, modelsoft->get_actuation(),
          std::make_shared<crocoddyl::CostModelSum>(*modelsoft->get_costs()));
  ...
  if (modelsoft->get_with_gravity_torque_reg()) {
    ...
    modelfree->get_costs()->addCost("grav_reg", cost,
                                    modelsoft->get_tau_grav_weight());
  }
  // optional armature
  if (armature.isZero(1e-9)) {
    modelfree->set_armature(
        Eigen::VectorXd::Zero(modelsoft->get_state()->get_nv()));
    modelsoft->set_armature(
        Eigen::VectorXd::Zero(modelsoft->get_state()->get_nv()));
  ...
  std::shared_ptr<crocoddyl::DifferentialActionDataAbstract> datafree =
      modelfree->createData();
  std::shared_ptr<crocoddyl::DifferentialActionDataAbstract> datasoft =
...
void test_calc_equivalent_free(DAMSoftContact3DTypes::Type action_type,
  ...
  std::shared_ptr<
      force_feedback_mpc::softcontact::DAMSoftContact3DAugmentedFwdDynamics>
      modelsoft = factory.create(action_type, ref_type);
  ...
  test_calc_free(modelsoft, 1e-3 * Eigen::VectorXd::Ones(
                                       modelsoft->get_state()->get_nv()));

  std::shared_ptr<
      force_feedback_mpc::softcontact::DAMSoftContact3DAugmentedFwdDynamics>
      modelsoft2 = factory.create(action_type, ref_type);
  ...
  test_calc_free(modelsoft2,
                 Eigen::VectorXd::Zero(modelsoft2->get_state()->get_nv()));
  // Test that freeFwdDyn = SoftContact(active=False) WITH armature
  test_calc_free(modelsoft2, 1e-3 * Eigen::VectorXd::Ones(
                                        modelsoft2->get_state()->get_nv()));
}
...
void test_calcDiff_equivalent_free(DAMSoftContact3DTypes::Type action_type,
  ...
  test_calcDiff_free(modelsoft,
                     Eigen::VectorXd::Zero(modelsoft->get_state()->get_nv()));
  // Test that freeFwdDyn = SoftContact(Kp=0;Kv=0), WITH armature
  test_calcDiff_free(modelsoft, 1e-3 * Eigen::VectorXd::Ones(
                                           modelsoft->get_state()->get_nv()));
  std::shared_ptr<
      force_feedback_mpc::softcontact::DAMSoftContact3DAugmentedFwdDynamics>
      modelsoft2 = factory.create(action_type, ref_type);
  ...
  modelsoft2->set_active_contact(false);
  // Test that freeFwdDyn = SoftContact(active=False) without armature
  test_calcDiff_free(modelsoft2,
                     Eigen::VectorXd::Zero(modelsoft2->get_state()->get_nv()));
  // Test that freeFwdDyn = SoftContact(active=False) WITH armature
  test_calcDiff_free(modelsoft2, 1e-3 * Eigen::VectorXd::Ones(
                                            modelsoft2->get_state()->get_nv()));
}
...
void register_action_model_unit_tests(
  ...
  ts->add(
      BOOST_TEST_CASE(boost::bind(&test_attributes, action_type, ref_type)));
  ts->add(BOOST_TEST_CASE(boost::bind(&test_partial_derivatives_against_numdiff,
                                      action_type, ref_type)));
  // Test equivalence with Euler for soft contact when Kp, Kv = 0 and f=0, or
  // active_contact = 0
  ts->add(BOOST_TEST_CASE(
      boost::bind(&test_calc_equivalent_free, action_type, ref_type)));
  ts->add(BOOST_TEST_CASE(
      boost::bind(&test_calcDiff_equivalent_free, action_type, ref_type)));
  // test partials with armature (Careful : error of size of u_drift and dtau_dx
  // in croco::DAMFree - need change from nu to nv !!!)
  ts->add(BOOST_TEST_CASE(
...
bool init_function() {
  for (size_t i = 0; i < DAMSoftContact3DTypes::all.size(); ++i) {
    register_action_model_unit_tests(DAMSoftContact3DTypes::all[i],
                                     pinocchio::LOCAL);
    register_action_model_unit_tests(DAMSoftContact3DTypes::all[i],
                                     pinocchio::WORLD);
    register_action_model_unit_tests(DAMSoftContact3DTypes::all[i],
