...
void exposeStateJerk() {
  bp::register_ptr_to_python<boost::shared_ptr<StateJerk>>();

  bp::class_<StateJerk, bp::bases<crocoddyl::StateAbstract>>(
      "StateJerk", "State model for low-pass filtered joints",
      bp::init<boost::shared_ptr<pinocchio::Model>, std::vector<int>, int>(
          bp::args("self", "model", "jerk_joint_ids", "nc"),
          "Initialize the state jerk model.\n\n"
          ":param model: pinocchio model\n"
          ":param jerk_joint_names: list of joints names that are low-pass "
          "filtered\n"
          ":param nc: contact force dimension")[bp::with_custodian_and_ward<1, 2>()])
      .def("zero", &StateJerk::zero, bp::args("self"),
           "Return the neutral robot configuration with zero velocity.\n\n"
           ":return neutral robot configuration with zero velocity")
      .def("rand", &StateJerk::rand, bp::args("self"),
           "Return a random reference state.\n\n"
           ":return random reference state")
      .def("diff", &StateJerk::diff_dx, bp::args("self", "x0", "x1"),
      ...
      .def("integrate", &StateJerk::integrate_x,
           bp::args("self", "x", "dx"),
      ...
      .def("Jintegrate", &StateJerk::Jintegrate_Js,
      ...
      .def("JintegrateTransport", &StateJerk::JintegrateTransport,
      ...
      .add_property(
          "pinocchio",
          bp::make_function(&crocoddyl::StateMultibody::get_pinocchio,
