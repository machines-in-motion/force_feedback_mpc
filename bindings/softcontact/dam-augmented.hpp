...
class DAMSoftContactAbstractAugmentedFwdDynamics_wrap
 ...
 public:
  DAMSoftContactAbstractAugmentedFwdDynamics_wrap(
      ...
      const pinocchio::FrameIndex frameId, const Eigen::VectorXd& Kp,
      const Eigen::VectorXd& Kv, const Eigen::Vector3d& oPc,
      const std::size_t nc,
      std::shared_ptr<crocoddyl::ConstraintModelManager> constraints = nullptr)
      : DAMSoftContactAbstractAugmentedFwdDynamics(
            state, actuation, costs, frameId, Kp, Kv, oPc, nc, constraints),
        bp::wrapper<DAMSoftContactAbstractAugmentedFwdDynamics>() {}
  ...
  cloneAsDouble() const {
    if (boost::python::override cloneAsDouble =
            this->get_override("cloneAsDouble")) {
      return bp::call<std::shared_ptr<crocoddyl::DifferentialActionModelBase>>(
          cloneAsDouble.ptr());
    }
    throw std::runtime_error(
        "cloneAsDouble() not implemented in Python derived class");
  }

  virtual std::shared_ptr<crocoddyl::DifferentialActionModelBase> cloneAsFloat()
      const {
    if (boost::python::override cloneAsFloat =
            this->get_override("cloneAsFloat")) {
      return bp::call<std::shared_ptr<crocoddyl::DifferentialActionModelBase>>(
          cloneAsFloat.ptr());
    }
    throw std::runtime_error(
        "cloneAsFloat() not implemented in Python derived class");
  }
};

// BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(DifferentialActionModel_quasiStatic_wraps,
//                                        DAMSoftContactAbstractAugmentedFwdDynamics::quasiStatic_x,
//                                        2, 4)
