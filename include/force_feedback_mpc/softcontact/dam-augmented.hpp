...
struct DADSoftContactAbstractAugmentedFwdDynamics
  ...
  explicit DADSoftContactAbstractAugmentedFwdDynamics(DAModel* const model)
      ...
      : DADBase(model),  // this complains ( no conversion error )
        ...
        fext(model->get_pinocchio().njoints,
             pinocchio::ForceTpl<double>::Zero()),
        ...
        tau_grav_residual_x(model->get_state()->get_nv(),
                            model->get_state()->get_ndx()),
        tau_grav_residual_u(model->get_state()->get_nv(),
  ...
  MatrixXs aba_dx;    //!< Partial derivative of ABA w.r.t. joint state
  ...
  Vector3s ov;  //!< Linear spatial velocity of the contact frame in
                //!< LOCAL_WORLD_ALIGNED
  Vector3s oa;  //!< Linear spatial acceleration of the contact frame in
                //!< LOCAL_WORLD_ALIGNED
  // Partials of frame spatial velocity w.r.t. joint pos, vel, acc
  MatrixXs lv_dq;  //!< Partial derivative of spatial velocity of the contact
                   //!< frame w.r.t. joint positions in LOCAL
  MatrixXs lv_dv;  //!< Partial derivative of spatial velocity of the contact
                   //!< frame w.r.t. joint velocities in LOCAL
  MatrixXs
      lv_dx;  //!< Partial derivative of spatial velocity of the contact
              //!< frame w.r.t. joint state (positions, velocities) in LOCAL
  // Partials of frame spatial acceleration w.r.t. joint pos, vel, acc
  MatrixXs v_dv;  //!< Partial derivative of spatial velocity of the contact
                  //!< frame w.r.t. joint velocity in LOCAL (not used)
  MatrixXs a_dq;  //!< Partial derivative of spatial acceleration of the contact
                  //!< frame w.r.t. joint positions in LOCAL
  MatrixXs a_dv;  //!< Partial derivative of spatial acceleration of the contact
                  //!< frame w.r.t. joint velocities in LOCAL
  MatrixXs a_da;  //!< Partial derivative of spatial acceleration of the contact
                  //!< frame w.r.t. joint accelerations in LOCAL
  // Partial of frame spatial acc w.r.t. state
  MatrixXs
      da_dx;  //!< Partial derivative of spatial acceleration of the contact
              //!< frame w.r.t. joint state (positions, velocities) in LOCAL
  MatrixXs da_du;  //!< Partial derivative of spatial acceleration of the
                   //!< contact frame w.r.t. joint torques in LOCAL
  MatrixXs da_df;  //!< Partial derivative of spatial acceleration of the
                   //!< contact frame w.r.t. contact force in LOCAL
  // Current force and next force
  VectorXs fout;  //!< Contact force time-derivative (output of the soft contact
                  //!< forward dynamics)
  VectorXs fout_copy;  //!< Contact force time-derivative (output of the soft
                       //!< contact forward dynamics) (copy)
  // Spatial wrench due to contact force
  pinocchio::ForceTpl<double>
      pinForce;  //!< External spatial force in body
                 //!< coordinates (at parent joint level)
  pinocchio::container::aligned_vector<pinocchio::ForceTpl<double>>
  ...
  MatrixXs dfdt_dx;  //!< Partial derivative of fout w.r.t. joint state
                     //!< (positions, velocities)
  ...
  MatrixXs
      dfdt_dx_copy;  //!< Partial derivative of fout w.r.t. joint state (copy)
  MatrixXs
      dfdt_du_copy;  //!< Partial derivative of fout w.r.t. joint torques (copy)
  MatrixXs
      dfdt_df_copy;  //!< Partial derivative of fout w.r.t. contact force (copy)
 ...
 * in multibody systems (augmented dynamics including the contact force as a
 * state)
 *
...
class DAMSoftContactAbstractAugmentedFwdDynamics
 ...
 public:
   ...
   * @brief Compute the system acceleration, and cost value
   *
   * It computes the system acceleration using the soft contact
   * forward-dynamics.
   *
   * @param[in] data  Soft contact forward-dynamics data
   * @param[in] x     State point \f$\mathbf{x}\in\mathbb{R}^{ndx}\f$
  ...
  std::size_t get_nresidual() {
    return this->get_nr() + this->get_state()->get_nv() + 2 * this->get_nc();
  };
 ...
 protected:
  ...
  bool with_gravity_torque_reg_;    //!< Control regularization w.r.t. gravity
                                    //!< torque
