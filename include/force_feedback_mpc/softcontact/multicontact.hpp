...
class ViscoElasticContact3D {
 public:
  ViscoElasticContact3D(
      pinocchio::Model& model, pinocchio::FrameIndex frameId,
      const Eigen::Vector3d& oPc = Eigen::Vector3d::Zero(), double Kp = 10.0,
      double Kv = 0.0,
...
class ViscoElasticContact3dMultiple {
 public:
  ...
  std::vector<pinocchio::Force> calc(pinocchio::Data& data,
...
class ForceBoxConstraint {
 public:
  ForceBoxConstraint(pinocchio::FrameIndex frameId, const Eigen::Vector3d& lb,
...
class ForceConstraintManager {
 ...
 private:
  ...
  std::map<pinocchio::FrameIndex,
           std::vector<std::shared_ptr<FrictionConeConstraint>>>
      contact_to_friction_map;
  std::map<pinocchio::FrameIndex,
           std::vector<std::shared_ptr<ForceBoxConstraint>>>
      contact_to_box_map;
...
class ForceCost {
 public:
  ForceCost(pinocchio::Model& model, pinocchio::FrameIndex frameId,
            const Eigen::Vector3d& f_des, const Eigen::Matrix3d& f_weight,
            pinocchio::ReferenceFrame pinRef);
  ...
  double calc(pinocchio::Data& data, const Eigen::Vector3d& f,
              pinocchio::ReferenceFrame pinRefDyn);
  std::pair<Eigen::Vector3d, Eigen::Matrix3d> calcDiff(
...
class ForceCostManager {
 public:
  ...
  std::pair<Eigen::VectorXd, Eigen::MatrixXd> calcDiff(
 ...
 private:
  ...
  std::map<pinocchio::FrameIndex, std::vector<std::shared_ptr<ForceCost>>>
      contact_to_cost_map;
  std::map<pinocchio::FrameIndex, int> contact_to_nr_map;
};
