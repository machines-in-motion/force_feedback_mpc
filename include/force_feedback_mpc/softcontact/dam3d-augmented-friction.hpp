 ...
 *
 * \sa `DAMSoftContact3DAugmentedFrictionFwdDynamicsTpl`, `calc()`,
 * `calcDiff()`, `createData()`
 */
...
class DAMSoftContact3DAugmentedFrictionFwdDynamicsTpl
 ...
 public:
  ...
  CROCODDYL_DERIVED_CAST(StateBase,
  ...
  int sign(const Scalar inVal) {
    return (int)((inVal > Scalar(0)) - (inVal < Scalar(0)));
  };
  Scalar sign_smooth(const Scalar inVal) {
    return std::tanh(eps_tanh_ * inVal);
  };
  Scalar sign_smooth_diff(const Scalar inVal) {
    return eps_tanh_ * (Scalar(1) - std::tanh(eps_tanh_ * inVal) *
                                        std::tanh(eps_tanh_ * inVal));
  };
...
template <typename _Scalar>
struct DADSoftContact3DAugmentedFrictionFwdDynamicsTpl
    : public sobec::DADSoftContactAbstractAugmentedFwdDynamicsTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  typedef _Scalar Scalar;
  ...
  explicit DADSoftContact3DAugmentedFrictionFwdDynamicsTpl(
  ...
  MatrixXs aba_df3d_copy;  //!< Partial derivative of ABA w.r.t. 3D contact
                           //!< force (copy)
  ...
  using Base::fext;  //!< External spatial forces in body coordinates (joint
                     //!< level)
  using Base::fext_copy;  //!< External spatial forces in body coordinates
                          //!< (joint level)
  using Base::pinForce;
  ...
  MatrixXs dfdt3d_dx;  //!< Partial derivative of fout3d w.r.t. joint state
  ...
  MatrixXs dfdt3d_dx_copy;  //!< Partial derivative of fout3d w.r.t. joint state
                            //!< (positions, velocities) (copy)
  MatrixXs dfdt3d_du_copy;  //!< Partial derivative of fout3d w.r.t. joint
                            //!< torques (copy)
  MatrixXs dfdt3d_df_copy;  //!< Partial derivative of fout3d w.r.t. contact
                            //!< force (copy)
