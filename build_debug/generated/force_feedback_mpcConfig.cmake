...
if(NOT force_feedback_mpc_FOUND)
  set(skip_this_file FALSE)
endif()
if(skip_this_file)
  foreach(component ${force_feedback_mpc_FIND_COMPONENTS})
    if(NOT "force_feedback_mpc_${component}_FOUND")
      set(skip_this_file FALSE)
    endif()
  endforeach()
endif()
...
set("force_feedback_mpc_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("FORCE_FEEDBACK_MPC_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("force_feedback_mpc_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/force_feedback_mpc/doxygen-html")
set("FORCE_FEEDBACK_MPC_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/force_feedback_mpc/doxygen-html")
set("force_feedback_mpc_DEPENDENCIES" "crocoddyl;eigenpy")
set("force_feedback_mpc_PKG_CONFIG_DEPENDENCIES" "example-robot-data")
...
set("_force_feedback_mpc_PKG_CONFIG_LIBDIR" "${pcfiledir}/../../lib")
set("_force_feedback_mpc_PKG_CONFIG_LIBS_LIST" "")
if(_force_feedback_mpc_PKG_CONFIG_LIBS_LIST)
  string(FIND ${_force_feedback_mpc_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  while(_is_comma_space GREATER -1)
    string(REPLACE ", " "," _force_feedback_mpc_PKG_CONFIG_LIBS_LIST "${_force_feedback_mpc_PKG_CONFIG_LIBS_LIST}")
    string(FIND ${_force_feedback_mpc_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  endwhile()
  string(REPLACE " " ";" _force_feedback_mpc_PKG_CONFIG_LIBS_LIST "${_force_feedback_mpc_PKG_CONFIG_LIBS_LIST}")
  set(LIBDIR_HINTS ${_force_feedback_mpc_PKG_CONFIG_LIBDIR})
  foreach(component ${_force_feedback_mpc_PKG_CONFIG_LIBS_LIST})
  ...
  endforeach()
endif(_force_feedback_mpc_PKG_CONFIG_LIBS_LIST)

set("force_feedback_mpc_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
set("FORCE_FEEDBACK_MPC_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
...
IF(COMMAND ADD_REQUIRED_DEPENDENCY)
  FOREACH(pkgcfg_dep ${force_feedback_mpc_PKG_CONFIG_DEPENDENCIES})
...
include("${CMAKE_CURRENT_LIST_DIR}/force_feedback_mpcTargets.cmake")
...
foreach(component ${force_feedback_mpc_FIND_COMPONENTS})
  ...
  if(EXISTS ${comp_file})
    ...
    set(force_feedback_mpc_${component}_FOUND FALSE)
  endif()
  if(force_feedback_mpc_${component}_FOUND)
    message(STATUS "force_feedback_mpc: ${component} found.")
  else()
    if(force_feedback_mpc_FIND_REQUIRED_${component})
      message(FATAL_ERROR "force_feedback_mpc: ${component} not found.")
    else()
      message(STATUS "force_feedback_mpc: ${component} not found.")
    endif()
  endif()
endforeach()
check_required_components("force_feedback_mpc")
