...
int main() {
  ...
  // runningModels; std::shared_ptr<crocoddyl::IntegratedActionModelEuler>
  // terminal_model; const double dt = 5e-2; const int T = 40;
  ...
  //     std::make_shared<crocoddyl::ConstraintModelManager>(state, nu); if(t !=
  //     0){
  ...
  //                                                                                                 );
  //     if (t != T){
  //         std::shared_ptr<crocoddyl::IntegratedActionModelEuler>
  //         running_model =
  ...
  // }
  // double const std_dev_SQP = std::sqrt((duration_SQP.array() -
  // duration_SQP.mean()).square().sum() / (nb_SQP - 1));
  ...
  // }
  // double const std_dev_CSQP = std::sqrt((duration_CSQP.array() -
  // duration_CSQP.mean()).square().sum() / (nb_CSQP - 1));

  // std::cout << "  " << std::left << std::setw(42) << "UR5 CSQP (50 QP iters)"
  // << std::left
  //           << std::setw(15) <<  duration_CSQP.mean() << std::left <<
  //           std::setw(15)
  ...
  //     duration_CSQP[i] = timer.elapsed().user;
  // }
  // double const std_dev_CSQP1000 = std::sqrt((duration_CSQP.array() -
  // duration_CSQP.mean()).square().sum() / (nb_CSQP - 1));

  // std::cout << "  " << std::left << std::setw(42) << "UR5 CSQP (200 QP
  // iters)" << std::left
  //           << std::setw(15) <<  duration_CSQP.mean() << std::left <<
  //           std::setw(15)
