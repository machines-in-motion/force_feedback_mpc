...
class OCPDataHandlerLPF(OCPDataHandlerAbstract):
    ...
    def extract_data(self, xs, us):
        ...
        if "ctrlReg" in ocp_data["active_costs"]:
            ocp_data["ctrlReg_ref"].append(
                self.ocp.terminalModel.differential.costs.costs[
                    "ctrlReg"
                ].cost.residual.reference
            )
        if "ctrlRegGrav" in ocp_data["active_costs"]:
            ocp_data["ctrlRegGrav_ref"].append(
                pin_utils.get_u_grav(xs[-1][: ocp_data["nq"]], ocp_data["pin_model"])
            )
        return ocp_data
        ...
        for k, data in enumerate(OCP_DATA):
            ...
            if k == 0:
                if "y" in which_plots or which_plots == "all" or "all" in which_plots:
                    if "xs" in data.keys():
                        fig_x, ax_x = self.plot_ocp_state(
                            data,
                            label=labels[k],
                            marker=markers[k],
                            color=colors[k],
                ...
                if "w" in which_plots or which_plots == "all" or "all" in which_plots:
                    if "us" in data.keys():
                        fig_u, ax_u = self.plot_ocp_control(
                            data,
                            label=labels[k],
                            marker=markers[k],
                            color=colors[k],
                ...
                if "ee" in which_plots or which_plots == "all" or "all" in which_plots:
                    if "xs" in data.keys():
                        fig_ee_lin, ax_ee_lin = self.plot_ocp_endeff_linear(
                            data,
                            label=labels[k],
                            marker=markers[k],
                            color=colors[k],
                        ...
                        fig_ee_ang, ax_ee_ang = self.plot_ocp_endeff_angular(
                            data,
                            label=labels[k],
                            marker=markers[k],
                            color=colors[k],
                ...
                if "f" in which_plots or which_plots == "all" or "all" in which_plots:
                    if "fs" in data.keys():
                        fig_f, ax_f = self.plot_ocp_force(
                            data,
                            label=labels[k],
                            marker=markers[k],
                            color=colors[k],
...
class MPCDataHandlerLPF(MPCDataHandlerAbstract):
    ...
    def init_predictions(self):
        ...
        """
        self.state_pred = np.zeros(
            (self.N_plan, self.N_h + 1, self.ny)
        )  # Predicted states  ( xs : {x* = (q*, v*)} )
        self.ctrl_pred = np.zeros(
            (self.N_plan, self.N_h, self.nu)
        )  # Predicted torques ( us : {u*} )
        self.force_pred = np.zeros(
            (self.N_plan, self.N_h, 6)
        )  # Predicted EE contact forces
        self.state_des_PLAN = np.zeros(
            (self.N_plan + 1, self.ny)
        )  # Predicted states at planner frequency  ( x* interpolated at PLAN freq )
        self.ctrl_des_PLAN = np.zeros(
            (self.N_plan, self.nu)
        )  # Predicted torques at planner frequency ( u* interpolated at PLAN freq )
        self.force_des_PLAN = np.zeros(
            (self.N_plan, 6)
        )  # Predicted EE contact forces planner frequency
        self.state_des_CTRL = np.zeros(
            (self.N_ctrl + 1, self.ny)
        )  # Reference state at motor drivers freq ( x* interpolated at CTRL freq )
        self.ctrl_des_CTRL = np.zeros(
            (self.N_ctrl, self.nu)
        )  # Reference input at motor drivers freq ( u* interpolated at CTRL freq )
        self.force_des_CTRL = np.zeros(
            (self.N_ctrl, 6)
        )  # Reference EE contact force at motor drivers freq
        self.state_des_SIMU = np.zeros(
            (self.N_simu + 1, self.ny)
        )  # Reference state at actuation freq ( x* interpolated at SIMU freq )
        self.ctrl_des_SIMU = np.zeros(
            (self.N_simu, self.nu)
        )  # Reference input at actuation freq ( u* interpolated at SIMU freq )
        self.force_des_SIMU = np.zeros(
            (self.N_simu, 6)
    ...
    def init_measurements(self, y0):
        ...
        """
        self.state_mea_SIMU = np.zeros(
            (self.N_simu + 1, self.ny)
        )  # Measured states ( x^mea = (q, v) from actuator & PyB at SIMU freq )
        self.state_mea_no_noise_SIMU = np.zeros(
            (self.N_simu + 1, self.ny)
        )  # Measured states ( x^mea = (q, v) from actuator & PyB at SIMU freq ) without noise
        self.force_mea_SIMU = np.zeros((self.N_simu, 6))  # Measurec contact forces
        self.tau_mea_derivative_SIMU = np.zeros(
            (self.N_simu + 1, self.n_lpf)
        )  # Measured joint torque derivatives
    ...
    def init_sim_data(self, y0):
        ...
        self.N_plan = int(
            self.T_tot * self.plan_freq
        )  # Total number of planning steps in the simulation
        self.N_ctrl = int(
            self.T_tot * self.ctrl_freq
        )  # Total number of control steps in the simulation
        self.N_simu = int(
            self.T_tot * self.simu_freq
        )  # Total number of simulation steps
        self.T_h = self.N_h * self.dt  # Duration of the MPC horizon (s)
        self.dt_ctrl = float(1.0 / self.ctrl_freq)  # Duration of 1 control cycle (s)
        self.dt_plan = float(1.0 / self.plan_freq)  # Duration of 1 planning cycle (s)
        self.dt_simu = float(1.0 / self.simu_freq)  # Duration of 1 simulation cycle (s)
        self.OCP_TO_PLAN_RATIO = (
            self.dt_plan / self.dt
        )  # Ratio of replanning interval over OCP interval
    ...
    def record_predictions(self, nb_plan, ocpSolver):
        ...
        self.y_curr = ocpSolver.xs[
            0
        ]  # y0* = measured state    (q0*, v0*, tau0* ) = (q^mea, v^mea, tau^mea )
        ...
        if self.is_contact:
            self.force_pred[nb_plan, :, :] = np.array(
                [
                    ocpSolver.problem.runningDatas[i]
                    .differential.multibody.contacts.contacts[self.contactFrameName]
                    .f.vector
    ...
    def record_simu_cycle_measured(
        self, nb_simu, y_mea_SIMU, y_mea_no_noise_SIMU, f_mea_SIMU
    ):
        """
        ...
        self.tau_mea_derivative_SIMU[nb_simu, :] = (
            y_mea_no_noise_SIMU[self.nx :]
            - self.state_mea_no_noise_SIMU[nb_simu, self.nx :]
        ) / self.dt_simu
    ...
    def record_plan_cycle_desired(self, nb_plan):
        ...
        self.state_des_PLAN[nb_plan + 1, :] = self.y_curr + self.OCP_TO_PLAN_RATIO * (
            self.y_pred - self.y_curr
        )
        if self.is_contact:
            self.force_des_PLAN[nb_plan, :] = self.f_curr + self.OCP_TO_PLAN_RATIO * (
                self.f_pred - self.f_curr
            )
    ...
    def extract_data(self, frame_of_interest):
        ...
        for i in range(plot_data["N_simu"] + 1):
            plot_data["grav"][i, :] = pin_utils.get_u_grav(
                plot_data["q_mea"][i, :], plot_data["pin_model"]
            )
        ...
        plot_data["lin_pos_ee_mea"] = pin_utils.get_p_(
            plot_data["q_mea"], self.rmodel, self.id_endeff
        )
        plot_data["lin_vel_ee_mea"] = pin_utils.get_v_(
            plot_data["q_mea"], plot_data["v_mea"], self.rmodel, self.id_endeff
        )
        plot_data["lin_pos_ee_mea_no_noise"] = pin_utils.get_p_(
            plot_data["q_mea_no_noise"], plot_data["pin_model"], self.id_endeff
        )
        plot_data["lin_vel_ee_mea_no_noise"] = pin_utils.get_v_(
            plot_data["q_mea_no_noise"],
            plot_data["v_mea_no_noise"],
            plot_data["pin_model"],
            self.id_endeff,
        )
        # Angular
        ...
        plot_data["ang_pos_ee_mea_no_noise"] = pin_utils.get_rpy_(
            plot_data["q_mea_no_noise"], plot_data["pin_model"], self.id_endeff
        )
        plot_data["ang_vel_ee_mea_no_noise"] = pin_utils.get_w_(
            plot_data["q_mea_no_noise"],
            plot_data["v_mea_no_noise"],
            plot_data["pin_model"],
            self.id_endeff,
        ...
        # EE des
        # Linear
        plot_data["lin_pos_ee_des_PLAN"] = pin_utils.get_p_(
            plot_data["q_des_PLAN"], self.rmodel, self.id_endeff
        )
        plot_data["lin_vel_ee_des_PLAN"] = pin_utils.get_v_(
            plot_data["q_des_PLAN"],
            plot_data["v_des_PLAN"],
            self.rmodel,
            self.id_endeff,
        )
        # Angular
        plot_data["ang_pos_ee_des_PLAN"] = pin_utils.get_rpy_(
            plot_data["q_des_PLAN"], self.rmodel, self.id_endeff
        )
        plot_data["ang_vel_ee_des_PLAN"] = pin_utils.get_w_(
            plot_data["q_des_PLAN"],
            plot_data["v_des_PLAN"],
            self.rmodel,
            self.id_endeff,
    ...
    def plot_mpc_results(
        ...
        if (
            "state" in which_plots
            or which_plots is None
            or which_plots == "all"
            or "all" in which_plots
        ):
            figs["state"], axes["state"] = self.plot_mpc_state(
                plot_data,
        ...
        if (
            "control" in which_plots
            or which_plots is None
            or which_plots == "all"
            or "all" in which_plots
        ):
            figs["control"], axes["control"] = self.plot_mpc_control(
                plot_data,
                PLOT_PREDICTIONS=PLOT_PREDICTIONS,
                pred_plot_sampling=pred_plot_sampling,
                SAVE=SAVE,
        ...
        if (
            ...
            figs["ee_lin"], axes["ee_lin"] = self.plot_mpc_endeff_linear(
                plot_data,
                PLOT_PREDICTIONS=PLOT_PREDICTIONS,
                pred_plot_sampling=pred_plot_sampling,
                SAVE=SAVE,
            ...
            figs["ee_ang"], axes["ee_ang"] = self.plot_mpc_endeff_angular(
                plot_data,
                PLOT_PREDICTIONS=PLOT_PREDICTIONS,
                pred_plot_sampling=pred_plot_sampling,
                SAVE=SAVE,
        ...
        if (
            "force" in which_plots
            or which_plots is None
            or which_plots == "all"
            or "all" in which_plots
        ):
            figs["force"], axes["force"] = self.plot_mpc_force(
                plot_data,
                PLOT_PREDICTIONS=PLOT_PREDICTIONS,
                pred_plot_sampling=pred_plot_sampling,
                SAVE=SAVE,
        ...
        if (
            ...
            if "K_diag" in plot_data.keys():
                figs["K_diag"], axes["K_diag"] = self.plot_mpc_ricatti_diag(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
            ...
            if "K_svd" in plot_data.keys():
                figs["K_svd"], axes["K_svd"] = self.plot_mpc_ricatti_svd(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
        ...
        if (
            ...
            if "V_diag" in plot_data.keys():
                figs["V_diag"], axes["V_diag"] = self.plot_mpc_Vxx_diag(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
            ...
            if "V_eig" in plot_data.keys():
                figs["V_eig"], axes["V_eig"] = self.plot_mpc_Vxx_eig(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
        ...
        if (
            ...
            if "S" in plot_data.keys():
                figs["S"] = self.plot_mpc_solver_reg(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
        ...
        if (
            ...
            if "J" in plot_data.keys():
                figs["J"], axes["J"] = self.plot_mpc_jacobian(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
        ...
        if (
            ...
            if "Q_diag" in plot_data.keys():
                figs["Q_diag"], axes["Q_diag"] = self.plot_mpc_Quu_diag(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
            ...
            if "Q_eig" in plot_data.keys():
                figs["Q_eig"], axes["Q_eig"] = self.plot_mpc_Quu_eig(
                    plot_data,
                    SAVE=SAVE,
                    SAVE_DIR=SAVE_DIR,
                    SAVE_NAME=SAVE_NAME,
        ...
        if (
            "solver" in which_plots
            or which_plots is None
            or which_plots == "all"
            or "all" in which_plots
        ):
            figs["solver"] = self.plot_mpc_solver(
                plot_data, SAVE=SAVE, SAVE_DIR=SAVE_DIR, SAVE_NAME=SAVE_NAME, SHOW=False
            )
    ...
    def plot_mpc_state(
        ...
        for i in range(nq):
            if PLOT_PREDICTIONS:
                ...
                for j in range(0, N_plan, pred_plot_sampling):
                    ...
                    tspan_y_pred = np.array(
                        [
                            t0_horizon + sum(plot_data["dts"][:i])
                            for i in range(len(plot_data["dts"]))
                        ]
                    )  # np.linspace(t0_horizon, t0_horizon + T_h, N_h+1)
                    tspan_u_pred = np.array(
                        [
                            t0_horizon + sum(plot_data["dts"][:i])
                            for i in range(len(plot_data["dts"]) - 1)
                        ]
                    )  # np.linspace(t0_horizon, t0_horizon + T_h - dt_plan, N_h)
                    # Set up lists of (x,y) points for predicted positions and velocities
                    points_q = (
                        np.array([tspan_y_pred, q_pred_i[j, :]])
                        .transpose()
                        .reshape(-1, 1, 2)
                    )
                    points_v = (
                        np.array([tspan_y_pred, v_pred_i[j, :]])
                        .transpose()
                        .reshape(-1, 1, 2)
                    )
                    points_tau = (
                        np.array([tspan_y_pred, tau_pred_i[j, :]])
                        .transpose()
                        .reshape(-1, 1, 2)
                    )
            ...
            ax[i, 0].plot(
                t_span_plan,
                plot_data["q_des_PLAN"][:, i],
                color="b",
                linestyle="-",
                marker=".",
                label="Desired (PLAN rate)",
                alpha=0.1,
            )
            #   ax[i,0].plot(t_span_simu, plot_data['q_mea'][:,i], 'r', label='Measured', linewidth=1, alpha=0.1)
            ax[i, 0].plot(
                t_span_simu,
                plot_data["q_mea_no_noise"][:, i],
                color="r",
                marker=None,
                linestyle="-",
                label="Measured (no noise)",
                alpha=0.6,
            )
            ...
            if (
                "ctrlReg" in plot_data["WHICH_COSTS"]
                or "ctrlRegGrav" in plot_data["WHICH_COSTS"]
            ):
                ax[i, 2].plot(
                    t_span_plan[:-1],
        ...
        for i in range(nq):
            ax_q_ylim = 1.1 * max(
                np.max(np.abs(plot_data["q_mea_no_noise"][:, i])), TOL
            )
            ax_v_ylim = 1.1 * max(
                np.max(np.abs(plot_data["v_mea_no_noise"][:, i])), TOL
            )
            ax_tau_ylim = 1.1 * max(
                np.max(np.abs(plot_data["tau_mea_no_noise"][:, i])), TOL
            )
        ...
        # Titles
        fig.suptitle(
            "State = joint position ($q$), velocity ($v$), torque ($\\tau$)", size=18
        )
        # Save fig
        if SAVE:
            figs = {"x": fig}
    ...
    def plot_mpc_control(
        ...
        for i in range(nq):
            ...
            ax[i].plot(
                t_span_plan,
                plot_data["w_pred"][:, 0, i],
                color="b",
                marker=".",
                linestyle="-",
                label="Optimal control w0*",
                alpha=0.6,
            )
    ...
    def plot_mpc_ricatti_diag(
        self, plot_data, SAVE=False, SAVE_DIR="/tmp", SAVE_NAME=None, SHOW=True
    ):
        """
        Plot ricatti data
        Input:
        ...
        for i in range(nq):
            ...
            ax_K[i, 0].plot(
                t_span_plan_u,
                plot_data["Kp_diag"][:, 0, i],
                "b-",
                label="Diag of Ricatti (Kp)",
            )
            ...
            ax_K[i, 1].plot(
                t_span_plan_u,
                plot_data["Kv_diag"][:, 0, i],
                "b-",
                label="Diag of Ricatti (Kv)",
            )
            ...
            ax_K[i, 2].plot(
                t_span_plan_u,
                plot_data["Ktau_diag"][:, 0, i],
                "b-",
                label="Diag of Ricatti (K\\tau)",
            )
            ax_K[i, 2].set_ylabel(
                "$K\\tau_{}$".format(i) + "$_{}$".format(i), fontsize=12
            )
    ...
    def plot_mpc_Vxx_eig(
        ...
        for i in range(nq):
            ...
            ax_V[i, 0].plot(
                t_span_plan_u,
                plot_data["Vxx_eig"][:, 0, i],
                "b-",
                label="Vxx eigenvalue",
            )
