...
class OptimalControlProblemLPF(OptimalControlProblemAbstract):
    ...
    def __init__(self, robot, config, lpf_joint_names):
        ...
        self.lpf_joint_ids, self.lpf_state_ids = getJointAndStateIds(
            self.rmodel, self.lpf_joint_names
        )
    ...
    def create_constraint_model_manager(self, state, actuation, node_id):
        ...
        if "ctrlBox" in self.WHICH_CONSTRAINTS and node_id != self.N_h:
            ...
            logger.debug(
                "constraint manager ctrl box "
                + str(node_id)
                + " lb = : "
                + str(constraintModelManager.g_lb)
            )
            logger.debug(
                "constraint manager ctrl box "
                + str(node_id)
                + " ub = : "
                + str(constraintModelManager.g_ub)
            )
        # End-effector position limits
        if "translationBox" in self.WHICH_CONSTRAINTS and node_id != 0:
            translationBoxConstraint = self.create_translation_constraint(
                state, actuation
            )
            constraintModelManager.addConstraint(
                "translationBox", translationBoxConstraint
            )
        # Contact force
        if (
            "forceBox" in self.WHICH_CONSTRAINTS
            and node_id != 0
            and node_id != self.N_h
        ):
        ...
        if "collisionBox" in self.WHICH_CONSTRAINTS:
            collisionBoxConstraints = self.create_collision_constraints(
                state, actuation
            )
            for i, collisionBoxConstraint in enumerate(collisionBoxConstraints):
                constraintModelManager.addConstraint(
                    "collisionBox_" + str(i), collisionBoxConstraint
                )

        return constraintModelManager

    def create_differential_action_model(
        self, state, actuation, constraintModelManager=None
    ):
        """
        ...
        # Otherwise just create free DAM
            ...
            if constraintModelManager is None:
                dam = crocoddyl.DifferentialActionModelFreeFwdDynamics(
                    state, actuation, crocoddyl.CostModelSum(state, nu=actuation.nu)
                )
            else:
                dam = crocoddyl.DifferentialActionModelFreeFwdDynamics(
                    state,
    ...
    def init_running_model(
        self, state, actuation, runningModel, contactModels, w_reg_ref
    ):
        """
        ...
        runningModel.set_control_reg_cost(
            self.wRegWeight, w_reg_ref[self.lpf_state_ids]
        )
        ...
        if len(contactModels) > 0:
            for k, contactModel in enumerate(contactModels):
                runningModel.differential.contacts.addContact(
                    self.contacts[k]["contactModelFrameName"],
                    contactModel,
                    active=self.contacts[k]["active"],
                )
    ...
    def init_terminal_model(self, state, actuation, terminalModel, contactModels):
        ...
        if "stateReg" in self.WHICH_COSTS:
            xRegCost = self.create_state_reg_cost(state, actuation)
            terminalModel.differential.costs.addCost(
                "stateReg", xRegCost, self.stateRegWeightTerminal * self.dt
            )
        # Ctrl regularization
        if "ctrlReg" in self.WHICH_COSTS:
            uRegCost = self.create_ctrl_reg_cost(state)
            terminalModel.differential.costs.addCost(
                "ctrlReg", uRegCost, self.ctrlRegWeightTerminal * self.dt
            )
        # Control regularization (gravity)
        if "ctrlRegGrav" in self.WHICH_COSTS:
            uRegGravCost = self.create_ctrl_reg_grav_cost(state)
            terminalModel.differential.costs.addCost(
                "ctrlRegGrav", uRegGravCost, self.ctrlRegGravWeightTerminal * self.dt
            )
        # Control limit
        if "ctrlLim" in self.WHICH_COSTS:
            uLimCost = self.create_ctrl_limit_cost(state)
            terminalModel.differential.costs.addCost(
                "ctrlLim", uLimCost, self.ctrlLimWeightTerminal * self.dt
            )
        # State limits
        if "stateLim" in self.WHICH_COSTS:
            xLimitCost = self.create_state_limit_cost(state, actuation)
            terminalModel.differential.costs.addCost(
                "stateLim", xLimitCost, self.stateLimWeightTerminal * self.dt
            )
        # EE placement
        if "placement" in self.WHICH_COSTS:
            framePlacementCost = self.create_frame_placement_cost(state, actuation)
            terminalModel.differential.costs.addCost(
                "placement",
                framePlacementCost,
                self.framePlacementWeightTerminal * self.dt,
            )
        # EE velocity
        if "velocity" in self.WHICH_COSTS:
            frameVelocityCost = self.create_frame_velocity_cost(state, actuation)
            terminalModel.differential.costs.addCost(
                "velocity",
                frameVelocityCost,
                self.frameVelocityWeightTerminal * self.dt,
            )
        # EE translation
        if "translation" in self.WHICH_COSTS:
            frameTranslationCost = self.create_frame_translation_cost(state, actuation)
            terminalModel.differential.costs.addCost(
                ...
                self.frameTranslationWeightTerminal * self.dt,
            )
        # End-effector orientation
        if "rotation" in self.WHICH_COSTS:
            frameRotationCost = self.create_frame_rotation_cost(state, actuation)
            terminalModel.differential.costs.addCost(
                "rotation",
                frameRotationCost,
                self.frameRotationWeightTerminal * self.dt,
            )
        ...
        if len(contactModels):
            for k, contactModel in enumerate(contactModels):
                terminalModel.differential.contacts.addContact(
                    self.contacts[k]["contactModelFrameName"],
                    contactModel,
                    active=self.contacts[k]["active"],
                )
    ...
    def lpf_parameter_log(self):
        ...
        if self.LPF_TYPE == 1:
            alpha = 1.0 / float(1 + 2 * np.pi * self.f_c * self.dt)
    ...
    def success_log(self):
        ...
        if self.nb_contacts > 0:
            ...
            for ct in self.contacts:
                logger.info(
                    "      Found [ "
                    + str(ct["contactModelType"])
                    + " ] (Baumgarte stab. gains = "
                    + str(ct["contactModelGains"])
                    + " , active = "
                    + str(ct["active"])
                    + " )"
                )
    ...
    def parse_unfiltered_torque_reg_reference(self, y0):
        ...
        if not hasattr(self, "wRegRef"):
            logger.error(
                "Need to specify 'wRegRef' in YAML config. Please select in ['zero', 'tau0', 'gravity']"
            )
            ...
            if self.wRegRef == "zero":
                ...
                logger.error(
                    "Unknown 'wRegRef' in YAML config. Please select in ['zero', 'tau0', 'gravity']"
                )
    ...
    def initialize(self, y0):
        ...
        for i in range(self.N_h):
            ...
            if self.nb_constraints == 0:
                dam, contactModels = self.create_differential_action_model(
                    state, actuation
                )
            else:
                # Create constraint manager and constraints
                constraintModelManager = self.create_constraint_model_manager(
                    state, actuation, i
                )

                # Create DAM (Contact or FreeFwd), IAM Euler and initialize costs+contacts+constraints
                dam, contactModels = self.create_differential_action_model(
                    state, actuation, constraintModelManager
                )
            runningModels.append(
                force_feedback_mpc.IntegratedActionModelLPF(
                    dam,
            ...
            self.init_running_model(
                state, actuation, runningModels[i], contactModels, w_reg_ref
            )

        # Terminal model
        if self.nb_constraints == 0:
            dam_t, contactModels = self.create_differential_action_model(
                state, actuation
            )
        else:
            constraintModelManager = self.create_constraint_model_manager(
                state, actuation, self.N_h
            )
            dam_t, contactModels = self.create_differential_action_model(
                state, actuation, constraintModelManager
            )
        terminalModel = force_feedback_mpc.IntegratedActionModelLPF(
            dam_t,
            LPFJointNames=self.lpf_joint_names,
            stepTime=0.0,
