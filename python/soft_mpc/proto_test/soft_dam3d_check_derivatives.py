...
q0 = np.array([0.5, -1, 1.5, 0, 0, -0.5, 0])
...
assert (
    np.linalg.norm(laq - fdyn_local(model, data, frameId, x0, tau, Kp, Kv, oPc)) < 1e-4
)
...
assert (
    np.linalg.norm(oaq - fdyn_world(model, data, frameId, x0, tau, Kp, Kv, oPc)) < 1e-4
)
...
lv_partial_dq, lv_partial_dv = pin.getFrameVelocityDerivatives(
    model, data, frameId, pin.LOCAL
)
dlf_dx[:, :nq] = (
    -Kp * (lJ[:3] + pin.skew(oRf.T @ (data.oMf[frameId].translation - oPc)) @ lJ[3:])
    - Kv * lv_partial_dq[:3]
)
...
# compare numdiff
ldaq_dx_ND = numdiff(
    lambda x_: fdyn_local(model, data, frameId, x_, tau, Kp, Kv, oPc), x0
)
...
odaq_dx[:, :nq] = oaba_dq + data.Minv @ lJ[:3].T @ (
    oRf.T @ dof_dx[:, :nq] + pin.skew(oRf.T @ of) @ lJ[3:]
)
...
# compare numdiff
odaq_dx_ND = numdiff(
    lambda x_: fdyn_world(model, data, frameId, x_, tau, Kp, Kv, oPc), x0
)
# print(odaq_dx)
# print(ldaq_dx)
...
ldaq_df = (
    data.Minv @ lJ[:3].T @ model.frames[frameId].placement.rotation @ np.eye(3)
)  # np.block([np.eye(3), np.zeros((3,3))]).T
...
odaq_df = (
    data.Minv @ lJ[:3].T @ model.frames[frameId].placement.rotation @ oRf.T
)  # np.block([np.eye(3), np.zeros((3,3))]).T
...
def frameAcceleration_local(model, data, x, u, Kp, Kv, oPc):
    ...
    dam = DAM3DClassical(
        state, actuation, runningCostModel, frameId, Kp, Kv, oPc, pinRefFrame=pin.LOCAL
    )
    dad = dam.createData()
    dam.calc(dad, x, u)
    aq = dad.xout
...
lda_dx_ND = numdiff(
    lambda x_: frameAcceleration_local(model, data, x_, tau, Kp, Kv, oPc), x0
)
...
lda_du_ND = numdiff(
    lambda tau_: frameAcceleration_local(model, data, x0, tau_, Kp, Kv, oPc), tau
)
...
oda_dx_ND = numdiff(
    lambda x_: frameAcceleration_world(model, data, x_, tau, Kp, Kv, oPc), x0
)
assert np.linalg.norm(oda_dx_ND - oda_dx) < 1e-3
oda_du = oRf @ lda_du
oda_du_ND = numdiff(
    lambda tau_: frameAcceleration_world(model, data, x0, tau_, Kp, Kv, oPc), tau
)
...
frameAccPinWorld = pin.getFrameAcceleration(
    model, data, frameId, pin.LOCAL_WORLD_ALIGNED
).linear
...
_, _, _, oa_da = pin.getFrameAccelerationDerivatives(
    model, data, frameId, pin.LOCAL_WORLD_ALIGNED
)
...
def lambdaDot_local(model, data, frameId, x, tau, Kp, Kv, oPc):
    ...
    assert (
        np.linalg.norm(
            lv - pin.getFrameJacobian(model, data, frameId, pin.LOCAL)[:3] @ v
        )
        < 1e-4
    )
    df_dt = -Kp * lv - Kv * la
...
# print(lda_dx)
ldfdt_dx_ND = numdiff(
    lambda x_: lambdaDot_local(model, data, frameId, x_, tau, Kp, Kv, oPc), x0
)
assert np.linalg.norm(ldfdt_dx_ND - ldfdt_dx) < 1e-2
ldfdt_du = -Kv * lda_du
ldfdt_du_ND = numdiff(
    lambda tau_: lambdaDot_local(model, data, frameId, x0, tau_, Kp, Kv, oPc), tau
)
...
def lambdaDot_world(model, data, frameId, x, tau, Kp, Kv, oPc):
    ...
    assert (
        np.linalg.norm(
            ov
            - pin.getFrameJacobian(model, data, frameId, pin.LOCAL_WORLD_ALIGNED)[:3]
            @ v
        )
        < 1e-4
    )
...
odfdt_dx_ND = numdiff(
    lambda x_: lambdaDot_world(model, data, frameId, x_, tau, Kp, Kv, oPc), x0
)
assert np.linalg.norm(odfdt_dx_ND - odfdt_dx) < 1e-2
odfdt_du = oRf @ ldfdt_du
odfdt_du_ND = numdiff(
    lambda tau_: lambdaDot_world(model, data, frameId, x0, tau_, Kp, Kv, oPc), tau
)
...
ref = pin.LOCAL
dam = DAM3DClassical(
    state, actuation, runningCostModel, frameId, Kp, Kv, oPc, pinRefFrame=ref
)
dam2 = DAM3DAugmented(
    state, actuation, runningCostModel, frameId, Kp, Kv, oPc, pinRefFrame=ref
)
