...
dam_py = DAMSoftContactDynamics3D(
    state, actuation, runningCostModel, frameId, Kp, Kv, oPc, pinRefFrame=REF_FRAME
)
dam_py.set_force_cost(np.array([0.0, 0.0, 0.0]), 1e-2)
dad_py = dam_py.createData()
# C++ model (bindings)
import sobec

dam_cpp = sobec.sobec_pywrap.DifferentialActionModelSoftContact3DFwdDynamics(
    state, actuation, runningCostModel, frameId, Kp, Kv, oPc, REF_FRAME
)
dam_cpp.set_force_cost(np.array([0.0, 0.0, 0.0]), 1e-2)
...
# Create free model
dam_free = crocoddyl.DifferentialActionModelFreeFwdDynamics(
