...
oPc = oPf + np.array([0.05, 0.0, 0])  # + cm in x world np.random.rand(3)
...
fext0[model.frames[frameId].parent] = model.frames[frameId].placement.act(
    pin.Force(lf0, np.zeros(3))
)
...
uref = np.zeros(
    nq
)  # pin_utils.get_tau(q0, np.zeros(nv), np.zeros(nq), fext0, model, np.zeros(nq)) #np.random.rand(nq)
...
endeff_translation = (
    oPc  # np.array([-0.4, 0.3, 0.7]) # move endeff +10 cm along x in WORLD frame
)
...
frameVelocityResidual = crocoddyl.ResidualModelFrameVelocity(
    state, endeff_frame_id, pin.Motion.Zero(), pin.WORLD
)
...
dam = sobec.DifferentialActionModelSoftContact3DFwdDynamics(
    state, actuation, runningCostModel, frameId, Kp, Kv, oPc, REF_FRAME
)
dam.set_force_cost(np.array([0.0, 0.0, 0.0]), 1e-2)
# dam_t = DAMSoftContactDynamics3D(state, actuation, terminalCostModel, frameId, Kp, Kv, oPc, pinRefFrame=REF_FRAME)
dam_t = sobec.DifferentialActionModelSoftContact3DFwdDynamics(
    state, actuation, terminalCostModel, frameId, Kp, Kv, oPc, REF_FRAME
)
...
terminalModel = crocoddyl.IntegratedActionModelEuler(dam_t, 0.0)
...
us_init = [
    pin_utils.get_tau(q0, v0, np.zeros(nq), fext0, model, np.zeros(nq))
    for i in range(T)
...
fs_lin = np.array(
    [
        data.oMf[frameId].rotation @ (-Kp * (ps[i, :] - oPc) - Kv * vs[i, :])
        for i in range(T)
    ]
)
fs_ang = np.zeros((T, 3))
