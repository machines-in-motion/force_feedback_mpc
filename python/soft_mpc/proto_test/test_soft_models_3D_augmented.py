...
from soft_mpc.soft_models_3D_augmented import (
    DAMSoftContactDynamics3D,
)
...
nu = nq
nx = nq + nv
q0 = np.array([0.1, 0.7, 0.0, 0.7, -0.5, 1.5, 0.0])  # np.random.rand(nq) #
...
oPc = oPf + np.array(
    [0.05, 0.0, 0]
)  # + cm in x world np.random.rand(3) #np.random.rand(3) #
...
dam_ND = DAMSoftContactDynamics3D(
    state, actuation, runningCostModel, frameId, Kp, Kv, oPc, pinRefFrame=pin.LOCAL
)
