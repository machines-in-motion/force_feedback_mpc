...
assert (
    np.linalg.norm(laq - fdyn_local(model, data, frameId, x0, tau, Kp, Kv, oPc, mask))
    < 1e-4
)
...
assert (
    np.linalg.norm(oaq - fdyn_world(model, data, frameId, x0, tau, Kp, Kv, oPc, mask))
    < 1e-4
)
...
# ND
dlf_dx_ND = numdiff(
    lambda x_: force_local(model, data, frameId, x_, Kp, Kv, oPc, mask), x0
)
dof_dx_ND = numdiff(
    lambda x_: force_world(model, data, frameId, x_, Kp, Kv, oPc, mask), x0
)
# Analytic
# local
...
# compare numdiff
ldaq_dx_ND = numdiff(
    lambda x_: fdyn_local(model, data, frameId, x_, tau, Kp, Kv, oPc, mask), x0
)
...
odaq_dx[:, :nq] = oaba_dq_copy + data.Minv @ lJ[:3].T @ (
    oRf.T[:, mask] @ dof_dx[:, :nq] + pin.skew(oRf.T @ fWORLD) @ lJ[3:]
)
odaq_dx[:, nq:] = oaba_dv_copy + data.Minv @ lJ[:3].T @ (
    oRf.T[:, mask] @ dof_dx[:, nq:]
)
odaq_du = oaba_dtau
# compare numdiff
odaq_dx_ND = numdiff(
    lambda x_: fdyn_world(model, data, frameId, x_, tau, Kp, Kv, oPc, mask), x0
)
...
dam = DAMSoftContactDynamics1D(
    state, actuation, runningCostModel, frameId, "1Dz", Kp, Kv, oPc, pinRefFrame=ref
)
...
endeff_frame_id = model.getFrameId("gripper_left_fingertip_1_link")
endeff_translation = oPc
...
# free model
damf = crocoddyl.DifferentialActionModelFreeFwdDynamics(
...
# soft contact (free)
damc2 = DAMSoftContactDynamics1D(
    state,
    actuation,
    runningCostModel,
    frameId,
...
tamf = crocoddyl.IntegratedActionModelEuler(damf, 0.0)
# tamc1 = crocoddyl.IntegratedActionModelEuler(damc1, 0.)
tamc2 = crocoddyl.IntegratedActionModelEuler(damc2, 0.0)
...
us_init = [
    pin_utils.get_tau(q0, v0, np.zeros(nq), lfext0, model, np.zeros(nq))
    for i in range(N)
]  # ddp.problem.quasiStatic(xs_init[:-1])
...
ddpf.setCallbacks([crocoddyl.CallbackLogger(), crocoddyl.CallbackVerbose()])
# ddpc1.setCallbacks([crocoddyl.CallbackLogger(),
#                 crocoddyl.CallbackVerbose()])
ddpc2.setCallbacks([crocoddyl.CallbackLogger(), crocoddyl.CallbackVerbose()])
