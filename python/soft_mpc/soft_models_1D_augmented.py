...
class StateSoftContact1D(crocoddyl.StateAbstract):
    def __init__(self, rmodel, nc):
        crocoddyl.StateAbstract.__init__(
            self, rmodel.nq + rmodel.nv + nc, 2 * rmodel.nv + nc
        )
    ...
    def Jintegrate(self, y, dy, Jfirst):
        ...
        Default values :
         ...
         op = crocoddyl.addto
        """
        Jfirst[: self.nv, : self.nv] = pin.dIntegrate(
            self.pinocchio, y[: self.nq], dy[: self.nv], pin.ARG0
        )  # , crocoddyl.addto)
    ...
    def JintegrateTransport(self, y, dy, Jin, firstsecond):
        if firstsecond == crocoddyl.Jcomponent.first:
            pin.dIntegrateTransport(
                self.pinocchio, y[: self.nq], dy[: self.nv], Jin[: self.nv], pin.ARG0
            )
        elif firstsecond == crocoddyl.Jcomponent.second:
            pin.dIntegrateTransport(
                self.pinocchio, y[: self.nq], dy[: self.nv], Jin[: self.nv], pin.ARG1
            )
        else:
...
class IAMSoftContactDynamics1D(
    ...
    def __init__(self, dam, dt=1e-3, withCostResidual=True):
        ...
        crocoddyl.ActionModelAbstract.__init__(
            self,
            crocoddyl.StateVector(dam.state.nq + dam.state.nv + 1),
            dam.nu,
            dam.costs.nr + 1,
        )
    ...
    def calcDiff(self, data, y, u):
        ...
        self.stateSoft.Jintegrate(
            y, data.dx, data.Fx
        )  # add identity to Fx = d(x+dx)/dx = d(q,v)/d(q,v)
...
class IADSoftContactDynamics1D(
    crocoddyl.ActionDataAbstract
...
class DAMSoftContactDynamics1D(crocoddyl.DifferentialActionModelAbstract):
    ...
    def __init__(
        ...
        crocoddyl.DifferentialActionModelAbstract.__init__(
            self, stateMultibody, actuationModel.nu, costModelSum.nr
        )
        self.Kp = Kp
        self.Kv = Kv
    ...
    def calc(self, data, x, f, u):
        ...
        pin.forwardKinematics(
            self.pinocchio, data.pinocchio, q, v, np.zeros(self.state.nq)
        )
        ...
        if self.active_contact:
            ...
            if self.pinRef != pin.LOCAL:
                data.fext[self.parentId] = self.jMf.act(
                    pin.Force(oRf.T @ fLOCAL, np.zeros(3))
                )
            data.xout = pin.aba(
                self.pinocchio,
                data.pinocchio,
            ...
            la = pin.getFrameAcceleration(
                self.pinocchio, data.pinocchio, self.frameId, pin.LOCAL
            ).linear
    ...
    def calcDiff(self, data, x, f, u):
        ...
        if self.active_contact:
            ...
            data.dABA_df = (
                data.pinocchio.Minv
                @ lJ[:3].T
                @ self.pinocchio.frames[self.frameId].placement.rotation
                @ np.eye(3)
            )
            ...
            if self.pinRef != pin.LOCAL:
                ...
                data.Fx[:, : self.state.nq] += (
                    data.pinocchio.Minv @ lJ[:3].T @ pin.skew(oRf.T @ f) @ lJ[3:]
                )
                # Rotate dABA/df
            ...
            da_dx[:, : self.state.nq] = (
                a_dq[:3] + a_da[:3] @ data.Fx[:, : self.state.nq]
            )  # same as aba_dq here
            da_dx[:, self.state.nq :] = (
                a_dv[:3] + a_da[:3] @ data.Fx[:, self.state.nq :]
            )  # same as aba_dv here
            ...
            if self.pinRef != pin.LOCAL:
                ...
                data.dfdt_dx[:, : self.state.nq] = (
                    oRf @ ldfdt_dx_copy[:, : self.state.nq]
                    - pin.skew(oRf @ data.fout_copy) @ oJ[3:]
                )
                data.dfdt_dx[:, self.state.nq :] = (
                    oRf @ ldfdt_dx_copy[:, self.state.nq :]
                )
...
class DADSoftContactDynamics(crocoddyl.DifferentialActionDataAbstract):
    ...
    def __init__(self, am):
        ...
        self.costs = am.costs.createData(
            crocoddyl.DataCollectorMultibody(self.pinocchio)
        )
