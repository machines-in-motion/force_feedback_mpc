...
class DAMSoftContactDynamics3D(crocoddyl.DifferentialActionModelAbstract):
        ...
        if self.active_contact:
            ...
            if self.pinRef != pin.LOCAL:
                ...
                data.fext[self.parentId] = self.jMf.act(
                    pin.Force(oRf.T @ data.f, np.zeros(3))
                )
            data.xout = pin.aba(
                self.pinocchio,
                data.pinocchio,
    ...
    def calcDiff(self, data, x, u):
        ...
        if self.active_contact:
            ...
            data.df_dx[:, self.state.nq :] = -self.Kv * lv_partial_dv[:3]
            # copy for later
            data.df_dx_copy = data.df_dx.copy()
            # rotate if not LOCAL
            if self.pinRef != pin.LOCAL:
                data.df_dx[:, : self.state.nq] = (
                    oRf @ data.df_dx_copy[:, : self.state.nq]
                    - pin.skew(oRf @ data.f_copy) @ oJ[3:]
                )
                data.df_dx[:, self.state.nq :] = (
                    oRf @ data.df_dx_copy[:, self.state.nq :]
                )
            # Computing the dynamics using ABA or manually if armature
            aba_dq, ada_dv, aba_dtau = pin.computeABADerivatives(
                self.pinocchio,
                data.pinocchio,
                q,
                v,
            ...
            data.Fx[:, : self.state.nq] = (
                aba_dq
                + data.pinocchio.Minv @ lJ[:3].T @ data.df_dx_copy[:, : self.state.nq]
            )
            data.Fx[:, self.state.nq :] = (
                ada_dv
                + data.pinocchio.Minv @ lJ[:3].T @ data.df_dx_copy[:, self.state.nq :]
