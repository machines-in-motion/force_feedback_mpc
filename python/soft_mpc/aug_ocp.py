...
class OptimalControlProblemSoftContactAugmented(OptimalControlProblemAbstract):
    ...
    def parse_constraints(self):
        ...
        if not hasattr(self, "WHICH_CONSTRAINTS"):
            ...
            if (
                self.WHICH_CONSTRAINTS == []
                or self.WHICH_CONSTRAINTS == [""]
                or self.WHICH_CONSTRAINTS == ["None"]
            ):
                self.nb_constraints = 0
            else:
    ...
    def create_differential_action_model(
        ...
        if softContactModel.nc == 3:
            ...
            if constraintModelManager is None:
                ...
                dam = force_feedback_mpc.DAMSoftContact3DAugmentedFwdDynamics(
                    state,
                    actuation,
            ...
            if constraintModelManager is None:
                dam = force_feedback_mpc.DAMSoftContact1DAugmentedFwdDynamics(
                    state,
                    actuation,
                    # crocoddyl.CostModelSum(state, nu=actuation.nu),
                    costModelSum,
        ...
        logger.warning(
            "Set dynamics and cost reference frames to "
            + str(softContactModel.pinRefFrame)
        )
        return dam

    def finalize_running_model(
        self, state, actuation, runningModel, softContactModel, node_id
    ):
        """
        ...
        if "force" in self.WHICH_COSTS:
            ...
            if softContactModel.nc == 3:
                ...
                forceRef = np.array(
                    [np.asarray(self.frameForceRef)[softContactModel.mask]]
                )
        ...
        if "forceRateReg" in self.WHICH_COSTS:
            ...
            runningModel.differential.f_rate_reg_weight = np.asarray(
                self.forceRateRegWeight
            )
        if "forceBox" in self.WHICH_CONSTRAINTS and node_id != 0:
            ...
            if softContactModel.nc == 3:
                ...
                runningModel.force_lb = np.array(
                    [np.asarray(self.forceLowerLimit)[softContactModel.mask]]
                )
                runningModel.force_ub = np.array(
                    [np.asarray(self.forceUpperLimit)[softContactModel.mask]]
                )
        if "frictionCone" in self.WHICH_CONSTRAINTS:
            ...
            try:
                ...
                logger.error(
                    "Soft contact model must be of dimension 3 to use the friction cone constraint"
                )
            fid = self.rmodel.getFrameId(self.frictionConeFrameName)
            residual = force_feedback_mpc.ResidualModelFrictionConeAugmented(
                state, fid, self.frictionCoefficient, actuation.nu
            )
            runningModel.friction_constraints = [residual]

    def finalize_terminal_model(
        self, state, actuation, terminalModel, softContactModel
    ):
        """
        ...
        if "force" in self.WHICH_COSTS:
            ...
            terminalModel.differential.f_weight = (
                np.asarray(self.frameForceWeightTerminal) * self.dt
            )
        # Frame force rate reg cost
        if "forceRateReg" in self.WHICH_COSTS:
            self.check_attribute("forceRateRegWeight")
            terminalModel.differential.with_force_rate_reg_cost = True
            terminalModel.differential.f_rate_reg_weight = (
                np.asarray(self.forceRateRegWeight) * self.dt
            )
    ...
    def success_log(self, softContactModel):
        ...
        logger.info(
            "    SOFT CONTACT MODEL [ oPc="
            + str(softContactModel.oPc)
            + " , Kp="
            + str(softContactModel.Kp)
            + ", Kv="
            + str(softContactModel.Kv)
            + ", pinRefFrame="
            + str(softContactModel.pinRefFrame)
            + "]"
        )
    ...
    def initialize(self, y0, softContactModel):
        ...
        for i in range(self.N_h):
            ...
            if self.nb_constraints == 0:
                dam = self.create_differential_action_model(
                    state, actuation, costModelSum, softContactModel
                )
            else:
            ...
            runningModels.append(
                force_feedback_mpc.IAMSoftContactAugmented(dam, self.dt)
            )
            # import pdb; pdb.set_trace()
            self.finalize_running_model(
                state, actuation, runningModels[i], softContactModel, i
            )
        ...
        if self.nb_constraints == 0:
            dam_t = self.create_differential_action_model(
                state, actuation, costModelSum_t, softContactModel
            )
        else:
            constraintModelManager = self.create_constraint_model_manager(
        ...
        terminalModel = force_feedback_mpc.IAMSoftContactAugmented(dam_t, 0.0)
