...
Kp = 10
Kv = 10
oPc = np.array([0.65, 0.0, 0.01])
rmodel = robot.model
rdata = robot.data
# Initial conditions
q = np.array(
    [0.0, 1.05, 0.0, -1.13, 0.2, 0.79, 0.0]
)  # pin.randomConfiguration(robot.model)
...
for i in range(N):
        ...
        softContactModelsStack = ViscoElasticContact3d_Multiple(
            ...
            [
                ViscoElasticContact3D(
                    state, actuation, frameId, oPc, Kp, Kv, pin.LOCAL_WORLD_ALIGNED
                )
            ],
        )
    else:
    ...
    if CONSTRAINT:
        ...
        xlb = np.concatenate(
            [robot.model.lowerPositionLimit, [-np.inf] * robot.model.nv]
        )
        xub = np.concatenate(
            [robot.model.upperPositionLimit, [np.inf] * robot.model.nv]
        )
    ...
    if FORCE_COST:
        forceCostManager = ForceCostManager(
            [
                ForceCost(
                    state, frameId, np.array([0] * 3), 0.0, pin.LOCAL_WORLD_ALIGNED
                )
            ],
    ...
    if FORCE_C:
        forceConstraintManager = ForceConstraintManager(
            [
                ForceBoxConstraint(
                    frameId, np.array([-np.inf] * 3), np.array([np.inf] * 3)
                )
            ],
            softContactModelsStack,
        )
        ...
        forceConstraintManager = ForceConstraintManager(
            [FrictionConeConstraint(frameId, MU)], softContactModelsStack
        )
    elif BOTH_C:
        forceConstraintManager = ForceConstraintManager(
            [
                FrictionConeConstraint(frameId, MU),
                ForceBoxConstraint(
                    frameId, np.array([-np.inf] * 3), np.array([np.inf] * 3)
                ),
            ],
