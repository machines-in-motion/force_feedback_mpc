...
for t in range(N_ocp + 1):
    ...
    com_track = crocoddyl.CostModelResidual(
        state, com_activation, com_residual
    )  # What does it correspond to exactly?
    ...
    ef_activation = crocoddyl.ActivationModelWeightedQuad(np.array([1.0, 1.0, 1.0]))
    ...
    ef_rotation_residual = crocoddyl.ResidualModelFrameRotation(
        state, efId, rdata.oMf[efId].rotation, nu
    )
    ef_rot_activation = crocoddyl.ActivationModelWeightedQuad(np.array([1.0, 1.0, 1.0]))
    ef_rot_track = crocoddyl.CostModelResidual(
        state, ef_rot_activation, ef_rotation_residual
    )
    if t == N_ocp:
    ...
    lf_contact = ViscoElasticContact3D(
        state, actuation, lfFootId, rdata.oMf[lfFootId].translation, Kp, Kv, pinRef
    )
    rf_contact = ViscoElasticContact3D(
        state, actuation, rfFootId, rdata.oMf[rfFootId].translation, Kp, Kv, pinRef
    )
    lh_contact = ViscoElasticContact3D(
        state, actuation, lhFootId, rdata.oMf[lhFootId].translation, Kp, Kv, pinRef
    )
    rh_contact = ViscoElasticContact3D(
        state, actuation, rhFootId, rdata.oMf[rhFootId].translation, Kp, Kv, pinRef
    )
    ef_contact = ViscoElasticContact3D(
        state, actuation, efId, rdata.oMf[efId].translation, Kp, Kv, pinRef
    )
    # Stack models
    softContactModelsStack = ViscoElasticContact3d_Multiple(
        state, actuation, [lf_contact, rf_contact, lh_contact, rh_contact, ef_contact]
    )

    # Constraints stack
    # constraintModelManager = None
    constraintModelManager = crocoddyl.ConstraintModelManager(state, actuation.nu)
    uBoxCstr = crocoddyl.ConstraintModelResidual(
        state,
    ...
    xBoxCstr = crocoddyl.ConstraintModelResidual(
        state, crocoddyl.ResidualModelState(state, actuation.nu), xlb, xub
    )
    ...
    f_weight = np.array([1.0, 1.0, 1.0]) * 1e-3
    if t == N_ocp:
        forceCostManager = ForceCostManager(
            [ForceCost(state, efId, np.array([-25, 0.0, 0.0]), f_weight * dt, pinRef)],
            softContactModelsStack,
        )
    else:
        forceCostManager = ForceCostManager(
            [ForceCost(state, efId, np.array([-25, 0.0, 0.0]), f_weight, pinRef)],
            softContactModelsStack,
        )
    ...
    forceRateCostManager = ForceRateCostManager(
        state, actuation, softContactModelsStack, fdot_weights
    )
    ...
    iam = IAMSoftContactDynamics3D_Go2(
        dam, dt=dt, withCostResidual=True, forceConstraintManager=forceConstraintManager
    )
    runningModels += [iam]
