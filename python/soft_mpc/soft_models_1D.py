    ...
    def calc(self, data, x, u):
        ...
        if self.active_contact:
            ...
            data.f3d = (
                -self.Kp
                * oRf.T
                @ (data.pinocchio.oMf[self.frameId].translation - self.oPc)
                - self.Kv * lv
            )
            data.f = data.f3d[self.mask]
            fLOCAL = np.zeros(3)
            fLOCAL[self.mask] = data.f
            ...
            if self.pinRef != pin.LOCAL:
                data.f3d = (
                    -self.Kp * (data.pinocchio.oMf[self.frameId].translation - self.oPc)
                    - self.Kv * oRf @ lv
                )
                data.f = data.f3d[self.mask]
                assert np.linalg.norm(data.f3d - oRf @ data.f3d_copy) < 1e-4
                assert np.linalg.norm(oRf.T @ data.f3d - data.f3d_copy) < 1e-4
                data.fWORLD = np.zeros(3)
                data.fWORLD[self.mask] = data.f
    ...
    def calcDiff(self, data, x, u):
        ...
        if self.active_contact:
            ...
            oJ = pin.getFrameJacobian(
                self.pinocchio, data.pinocchio, self.frameId, pin.LOCAL_WORLD_ALIGNED
            )
            ...
            data.df3d_dx[:, self.state.nq :] = -self.Kv * lv_partial_dv[:3]
            ...
            if self.pinRef != pin.LOCAL:
                data.df3d_dx[:, : self.state.nq] = (
                    oRf @ data.df3d_dx_copy[:, : self.state.nq]
                    - pin.skew(oRf @ data.f3d_copy) @ oJ[3:]
                )
                data.df3d_dx[:, self.state.nq :] = (
                    oRf @ data.df3d_dx_copy[:, self.state.nq :]
                )
            ...
            # LWA case
                ...
                data.Fx[:, : self.state.nq] = aba_dq + data.pinocchio.Minv @ lJ[
                    :3
                ].T @ (
                    oRf.T[:, self.mask] @ data.df_dx[:, : self.state.nq]
                    + pin.skew(oRf.T @ data.fWORLD) @ lJ[3:]
                )
                data.Fx[:, self.state.nq :] = aba_dv + data.pinocchio.Minv @ lJ[
                    :3
                ].T @ (oRf.T[:, self.mask] @ data.df_dx[:, self.state.nq :])
