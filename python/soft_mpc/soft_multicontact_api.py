...
class ViscoElasticContact3D:
    def __init__(
        ...
        # Partial derivatives
        self.dABA_df3d = np.zeros(
            (self.state.nv, self.nc)
        )  # derivative of ada (ddq) w.r.t. 3d force
        self.dfdt_dx = np.zeros(
            (self.nc, self.state.ndx)
        )  # derivative of fdot (3D) w.r.t. state
        self.dfdt_du = np.zeros(
            (self.nc, self.actuation.nu)
        )  # derivative of fdot (3D) w.r.t. control
        self.dfdt_df = np.zeros(
            (self.nc, self.nc)
        )  # derivative of fdot (3D) w.r.t. forces
    ...
    def calc_fdot(self, dad):
        ...
        """
        la = pin.getFrameAcceleration(
            self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL
        ).linear
        lv = pin.getFrameVelocity(
            self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL
        ).linear
        ...
        if self.pinRef != pin.LOCAL:
            oa = pin.getFrameAcceleration(
                self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL_WORLD_ALIGNED
            ).linear
            ov = pin.getFrameVelocity(
                self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL_WORLD_ALIGNED
            ).linear
    ...
    def update_ABAderivatives(self, dad, f):
        ...
        if self.pinRef != pin.LOCAL:
            ...
            dad.Fx[:, : self.state.nv] += (
                dad.pinocchio.Minv @ lJ[:3].T @ pin.skew(oRf.T @ f) @ lJ[3:]
            )
            # Rotate dABA/df
    ...
    def calcDiff(self, dad):
        ...
        lv_dq, lv_dv = pin.getFrameVelocityDerivatives(
            self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL
        )
        lv_dx = np.hstack([lv_dq, lv_dv])
        _, a_dq, a_dv, a_da = pin.getFrameAccelerationDerivatives(
            self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL
        )
        da_dx = np.zeros((3, self.state.ndx))
        da_dx[:, : self.state.nv] = (
            a_dq[:3] + a_da[:3] @ dad.Fx[:, : self.state.nv]
        )  # same as aba_dq here
        da_dx[:, self.state.nv :] = (
            a_dv[:3] + a_da[:3] @ dad.Fx[:, self.state.nv :]
        )  # same as aba_dv here
        da_du = a_da[:3] @ dad.Fu
        da_df = (
            a_da[:3] @ dad.dABA_df
        )  # this dad attribute was computed in update update_ABAderivatives
        # Deriv of lambda dot
        ...
        if self.pinRef != pin.LOCAL:
            oJ = pin.getFrameJacobian(
                self.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL_WORLD_ALIGNED
            )
            self.dfdt_dx[:, : self.state.nv] = (
                oRf @ self.ldfdt_dx_copy[:, : self.state.nv]
                - pin.skew(oRf @ self.fout_copy) @ oJ[3:]
            )
            self.dfdt_dx[:, self.state.nv :] = (
                oRf @ self.ldfdt_dx_copy[:, self.state.nv :]
            )
...
class FrictionConeConstraint:
    def __init__(self, frameId, coef, normal="z", lb_slack=0.0):
        self.nc = 3
        self.nr = 1
        self.active = True
        self.frameId = frameId
...
class ForceConstraintManager:
    def __init__(self, constraints, contacts):
        ...
        for cstr in self.constraints:
            ...
            if found == False:
                print(
                    "ERROR : no contact model was found that matches the constraint on frame ",
                    cstr.frameId,
                )
            assert found == True
    ...
    def calcDiff(self, f):
        ...
        for ct in self.contacts:
            ...
            for cstr in self.contact_to_cstr_map[ct.frameId]:
                # Compute the constraint residual
                self.residual_df[nr_i : nr_i + cstr.nr, nc_i : nc_i + cstr.nc] = (
                    cstr.calcDiff(f[nc_i : nc_i + ct.nc])
                )
                nr_i += cstr.nr
            nc_i += ct.nc
        # For each constraint, stack residual
...
class ForceCost:
    ...
    def calcDiff(self, dad, f, pinRefDyn):
        ...
        oRf = dad.pinocchio.oMf[self.frameId].rotation
        lJ = pin.getFrameJacobian(
            self.state.pinocchio, dad.pinocchio, self.frameId, pin.LOCAL
        )
        # Compute force residual and add force cost to total cost
        if self.pinRef != pinRefDyn:
            if self.pinRef == pin.LOCAL:
                ...
                self.f_residual_x[:3, : self.state.nv] = pin.skew(
                    oRf @ f
                ) @ self.oJ.bottomRows(3)
                dad.Lx += (
                    self.f_residual.T
                    @ self.f_weight
                    @ pin.skew(oRf @ f)
...
class DAMSoftContactDynamics3D_Go2(crocoddyl.DifferentialActionModelAbstract):
    def __init__(
        ...
        crocoddyl.DifferentialActionModelAbstract.__init__(
            self, stateMultibody, actuationModel.nu, costModelSum.nr, ng, nh
        )
    ...
    def calc(self, data, x, f, u=None):
        ...
        if u is not None:
            ...
            pin.forwardKinematics(
                self.pinocchio, data.pinocchio, q, v, np.zeros(self.state.nv)
            )
            ...
            if self.active_contact:
                ...
                data.xout = pin.aba(
                    self.pinocchio, data.pinocchio, q, v, data.multibody.actuation.tau
                )
...
class IAMSoftContactDynamics3D_Go2(
    crocoddyl.ActionModelAbstract
):  # IntegratedActionModelAbstract
    def __init__(
        self, dam, dt=1e-3, withCostResidual=True, forceConstraintManager=None
    ):
        ...
        crocoddyl.ActionModelAbstract.__init__(
            self,
            force_feedback_mpc.StateSoftContact(dam.pinocchio, int(dam.nc_tot)),
            nu,
            nr,
            ng,
            0,
        ...
        self.stateSoft = force_feedback_mpc.StateSoftContact(
            dam.pinocchio, int(dam.nc_tot)
        )  # Custom state
        self.nf = dam.nc_tot  # total force space dimension
        self.ny = (
            self.stateSoft.nx
        )  # augmented state dimension (classical state + force)
        ...
        self.forceConstraints = (
            forceConstraintManager  # custom force constraint manager
        )
    ...
    def calcDiff(self, data, y, u=None):
        ...
        if u is not None:
            ...
            self.stateSoft.JintegrateTransport(
                y, data.dx, data.Fx, crocoddyl.Jcomponent.second
            )
            data.Fx += self.stateSoft.Jintegrate(
                y, data.dx, crocoddyl.Jcomponent.first
            ).tolist()[0]  # add identity to Fx = d(x+dx)/dx = d(q,v)/d(q,v)
            data.Fx[ndx:, ndx:] -= np.eye(
                nf
            )  # remove identity from Ftau (due to stateSoft.Jintegrate)
            self.stateSoft.JintegrateTransport(
                y, data.dx, data.Fu, crocoddyl.Jcomponent.second
            )
            ...
            if self.with_force_constraint:
                ...
                if len(data.Gx.shape) == 1:
                    ...
                    data.Gx[ng_dam : ng_dam + ng_f, ndx : ndx + self.nc_f] = (
                        self.forceConstraints.calcDiff(f)
                    )
...
class IADSoftContactDynamics3D_Go2(crocoddyl.ActionDataAbstract):
    ...
    def __init__(self, am):
        ...
        self.Luu = np.zeros(
            (am.differential.actuation.nu, am.differential.actuation.nu)
        )
        # Constraints
