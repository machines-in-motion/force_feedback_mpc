...
class DDPDataHandlerSoftContact(DDPDataHandlerClassical):
    ...
    def extract_data(self, ee_frame_name, ct_frame_name):
        ...
        if self.softContactModel.nc == 3:
            fs_lin = np.array(
                [
                    self.softContactModel.computeForce_(
                        ddp_data["pin_model"], xs[i, :nq], xs[i, nq : nq + nv]
                    )
                    for i in range(ddp_data["T"])
                ]
            )
            fdes_lin = np.array(
                [
                    self.ddp.problem.runningModels[i].differential.f_des
                    for i in range(ddp_data["T"])
                ]
            )
        else:
            fs_lin = np.zeros((ddp_data["T"], 3))
            fs_lin[:, self.softContactModel.mask] = np.array(
                [
                    self.softContactModel.computeForce_(
                        ddp_data["pin_model"], xs[i, :nq], xs[i, nq:]
                    )
                    for i in range(ddp_data["T"])
                ]
            )
...
class MPCDataHandlerSoftContact(MPCDataHandlerClassical):
    ...
    def record_predictions(self, nb_plan, ddpSolver, softContactModel):
        ...
        self.x_curr = self.state_pred[
            nb_plan, 0, :
        ]  # x0* = measured state    (q^,  v^ )
        self.x_pred = self.state_pred[
            nb_plan, 1, :
        ...
        if softContactModel.nc == 3:
            fs_lin = np.array(
                [
                    softContactModel.computeForce_(
                        self.rmodel, xs[i, : self.nq], xs[i, self.nq :]
                    )
            ...
            fs_lin[:, softContactModel.mask] = np.array(
                [
                    softContactModel.computeForce_(
                        self.rmodel, xs[i, : self.nq], xs[i, self.nq :]
                    )
