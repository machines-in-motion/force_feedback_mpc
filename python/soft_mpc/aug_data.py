...
class OCPDataHandlerSoftContactAugmented(OCPDataHandlerClassical):
    ...
    def extract_data(self, xs, us, model):
        ...
        if self.softContactModel.nc == 3:
            ...
            fdes_lin = np.array(
                [
                    self.ocp.runningModels[i].differential.f_des
                    for i in range(ocp_data["T"])
                ]
            )
        else:
            fs_lin = np.zeros((ocp_data["T"], 3))
            fs_lin[:, self.softContactModel.mask] = [
                xs[i, -1] for i in range(ocp_data["T"])
            ]
...
class MPCDataHandlerSoftContactAugmented(MPCDataHandlerClassical):
    ...
    def record_predictions(self, nb_plan, ocpSolver):
        ...
        self.y_curr = self.state_pred[
            nb_plan, 0, :
        ]  # y0* = measured state    (q^,  v^, f^ )
        self.y_pred = self.state_pred[
            nb_plan, 1, :
        ]  # y1* = predicted state   (q1*, v1*, f1*)
    ...
    def record_simu_cycle_measured(
        self, nb_simu, y_mea_SIMU, y_mea_no_noise_SIMU, tau_mea_SIMU
    ):
        """
        ...
        if nb_simu > 0:
            self.tau_mea_derivative_SIMU[nb_simu, :] = (
                tau_mea_SIMU - self.tau_mea_SIMU[nb_simu - 1, :]
            ) / self.dt_simu
    ...
    def plot_mpc_force(
        ...
        for i in range(3):
            ...
            ax[i].plot(
                t_span_simu, f_ee_mea, "r-", label="Measured", linewidth=2, alpha=0.6
            )
            # Plot reference
        ...
        if AUTOSCALE:
            ax_ylim1 = 1.1 * max(
                np.max(np.abs(plot_data["f_ee_pred"])), TOL
            )  # 1.1*max( np.nanmax(np.abs(plot_data['f_ee_mea'])), TOL )
