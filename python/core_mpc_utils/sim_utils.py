...
# Display
def display_ball(
    p_des, robot_base_pose=pin.SE3.Identity(), RADIUS=0.05, COLOR=[1.0, 1.0, 1.0, 1.0]
):
    """
...
def display_contact_surface(
    M, robotId=1, radius=0.5, length=0.0, bullet_endeff_ids=[], TILT=[0.0, 0.0, 0.0]
):
    """
...
def set_lateral_friction(bodyId, coef, linkId=-1):
    ...
    p.changeDynamics(
        bodyId,
        linkId,
        lateralFriction=coef,
        rollingFriction=coef,
        spinningFriction=coef,
    )
    logger.info(
        "Set friction of body n°"
        + str(bodyId)
        + " (link n°"
        + str(linkId)
        + ") to "
...
def set_contact_stiffness_and_damping(bodyId, Ks, Kd, linkId=-1):
    ...
    logger.info(
        "Set contact stiffness of body n°"
        + str(bodyId)
        + " (link n°"
        + str(linkId)
        + ") to "
...
def display_capsule(M, RADIUS, LENGTH, COLOR=[1.0, 0.0, 0.0, 1.0]) -> int:
    """
...
def setup_obstacle_collision(robot_simulator, pin_robot, config, TYPE="capsule"):
    ...
    if TYPE == "capsule":
        ...
        OBSTACLE_GEOM_OBJECT = create_caps_obstacle(
            OBSTACLE_POSE, OBSTACLE_RADIUS, OBSTACLE_LENGTH, name="obstacle1"
        )
    if TYPE == "box":
        ...
        OBSTACLE_POSE = pin.XYZQUATToSE3(OBSTACLE_XYZQUAT)
        OBSTACLE_RADIUS = config["OBSTACLE_RADIUS"]
        OBSTACLE_GEOM_OBJECT = create_box_obstacle(
            OBSTACLE_POSE, OBSTACLE_RADIUS, name="obstacle1"
        )
    ...
    robot_simulator.pin_robot.collision_model = transform_model_into_capsules(
