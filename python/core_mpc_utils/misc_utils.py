...
def parse_OCP_script(argv=None):
    ...
    PARSER.add_argument(
        "--PLOT", action="store_true", default=False, help="Plot OCP solution"
    )
    PARSER.add_argument(
        "--DISPLAY",
        action="store_true",
        default=False,
        help="Animate solution in Gepetto Viewer",
    )
...
def parse_MPC_script(argv=None):
    ...
    PARSER.add_argument(
        "--SAVE_DIR", type=str, default="/tmp/", help="Where to save the sim data"
    )
    return PARSER.parse_args(argv)
...
def moving_average(data, window_size):
    ...
    return np.pad(
        smoothed, (pad_width, data.size - smoothed.size - pad_width), mode="edge"
    )
