...
class AntiAliasingFilter:
    def __init__(self, filter_type="iir"):
        ...
        try:
            assert filter_type in supported_filter_types
        except:
            logger.error(
                "AntiAliasingFilter type should be in " + str(supported_filter_types)
            )
    ...
    def step(self, nb1, nb2, freq1, freq2, data_at_freq2):
        ...
        if filterSize == 0:
            ...
            data_at_freq1 = self.moving_average(
                data_at_freq2[nb2 - filterSize : nb2], filterSize
            )
...
class LowLevelTorqueController:
    def __init__(self, config, nu, use):
        ...
        logger.info(
            "Created LowLevelTorqueController(TORQUE_TRACKING="
            + str(self.TORQUE_TRACKING)
            + ")."
        )
    ...
    def step(self, reference_torque, measured_torque, measured_torque_derivative):
        ...
        if self.TORQUE_TRACKING and len(measured_torque) != 0:
            ...
            motor_torque -= 1.0 * self.gain_P.dot(self.err_P)
...
class ActuationModel:
    def __init__(self, config, nu, SEED=1):
        ...
        self.alpha = np.random.uniform(
            low=self.config["alpha_min"], high=self.config["alpha_max"], size=(nu,)
        )
        self.beta = np.random.uniform(
            low=self.config["beta_min"], high=self.config["beta_max"], size=(nu,)
        )
        # Delays
        ...
        self.DELAY_SIM = config[
            "DELAY_SIM"
        ]  # Add delay in reference torques (low-level)
        self.SCALE_TORQUES = config[
            "SCALE_TORQUES"
        ]  # Affinescaling of reference torque
        self.NOISE_TORQUES = config[
            "NOISE_TORQUES"
        ]  # Moving average smoothing of reference torques
        ...
        self.tau_sf_max = config[
            "static_friction_max_torque"
        ]  # Max. static friction torque
        self.tau_vf_slope = config[
            "viscous_friction_slope"
        ]  # Slope of the viscous friction torque w.r.t. velocity
        logger.info(
            "Created ActuationModel(DELAY_SIM="
            + str(self.DELAY_SIM)
            + ", SCALE_TORQUES="
            + str(self.SCALE_TORQUES)
            + ", NOISE_TORQUES="
            + str(self.NOISE_TORQUES)
            + ", STATIC_FRICTION="
            + str(self.STATIC_FRICTION)
            + ", VISCOUS_FRICTION="
            + str(self.VISCOUS_FRICTION)
            + ")."
        )
        if self.SCALE_TORQUES:
            logger.info(
                "Torques scaling : alpha = "
                + str(self.alpha)
                + " | beta = "
                + str(self.beta)
            )
    ...
    def step(self, motor_torque, joint_vel=None):
        ...
        if self.NOISE_TORQUES and len(measured_torque) != 0:
            noise_u = np.random.normal(0.0, self.var_u, self.nu)
...
class CommunicationModel:
    def __init__(self, config):
        ...
        self.DELAY_OCP = config[
            "DELAY_OCP"
        ]  # Add delay in OCP solution (i.e. ~1ms resolution time)
        logger.info(
            "Created CommunicationModel(DELAY_OCP=" + str(self.DELAY_OCP) + ")."
        )
    ...
    def step(self, predicted_state, current_control):
        ...
        if self.DELAY_OCP:
            delay = int(
                self.config["delay_OCP_ms"] * 1e-3 * self.config["plan_freq"]
            )  # in planning cycles
...
class SensorModel:
    def __init__(self, config, naug=0, SEED=1):
        ...
        # Sensing model options
        self.NOISE_STATE = config[
            "NOISE_STATE"
        ]  # Add Gaussian noise on the measured state
    ...
    def step(self, measured_state):
        ...
        if self.NOISE_STATE:
            noise_q = np.random.normal(0.0, self.var_q, self.nq)
            noise_v = np.random.normal(0.0, self.var_v, self.nv)
            if self.naug > 0:
                noise_tau = np.random.normal(0.0, self.var_aug, self.naug)
