...
IntegratedActionModelJerk::IntegratedActionModelJerk(
    boost::shared_ptr<DifferentialActionModelAbstract> model,
    std::vector<std::string> jerk_joint_names, 
    std::size_t nc,
    const double& time_step,
    const bool& with_cost_residual)
    : Base(model->get_state(), model->get_nu(),
           model->get_nr() + 2 * jerk_joint_names.size(),
           model->get_ng() + jerk_joint_names.size(),
           0),
      nw_(model->get_nu()),
      differential_(model),
  ...
  // Check that used-specified Jerk joints are valid (no free-flyer) and collect
  // ids
  ntau_ = jerk_joint_names.size();
  nc_ = nc.size();
  for (std::vector<std::string>::iterator iter = jerk_joint_names.begin();
       iter != jerk_joint_names.end(); ++iter) {
  ...
  lpf_joint_names_ = jerk_joint_names;
  // Get jerk torque ids
  // fixed base
  ...
  for (std::vector<std::string>::iterator joint_names_iter =
    ...
    std::vector<std::string>::iterator it = std::find(
        jerk_joint_names.begin(), jerk_joint_names.end(), *joint_names_iter);
    if (it == jerk_joint_names.end()) {
  ...
  // Set constraint bounds (add ntau dimension for jerk torques constraints)
...
void IntegratedActionModelJerk::calc(
  ...
  d->tau_tmp(non_lpf_torque_ids_) =
      w(non_lpf_torque_ids_);                   // NON-Jerk dimensions
  d->tau_tmp(lpf_torque_ids_) = y.tail(ntau_);  // Jerk dimensions
#else
  ...
  // std::cout << "[jerk.calc] tau = " << tau << std::endl;
  ...
  // Add hard-coded cost on unfiltered torque a[r(w)] only at Jerk joints
  // Torque REG
  if (tauReg_weight_ > 0) {
#if EIGEN_VERSION_AT_LEAST(3, 4, 0)
...
#endif
    // add to cost
    d->cost += double(0.5 * time_step_ * tauReg_weight_ *
  ...
  // Hard-code Jerk torque constraint residual here
...
void IntegratedActionModelJerk::calcDiff(
    const boost::shared_ptr<ActionDataAbstract>& data,
    const Eigen::Ref<const VectorXs>& y, const Eigen::Ref<const VectorXs>& w) {
  const std::size_t& nv = differential_->get_state()->get_nv();
  const std::size_t& nx = differential_->get_state()->get_nx();
  ...
  if (!tau_plus_integration_) {
    ...
    // Partial blocks of Jerk dimensions due y+ dependency on tau
#if EIGEN_VERSION_AT_LEAST(3, 4, 0)
...
#endif
    // Jerk partial block
    d->Fy.bottomRightCorner(ntau_, ntau_).diagonal().array() = double(alpha_);
    ...
    d->Fy.bottomRightCorner(ntau_, ntau_).diagonal().array() -= double(1.);

    // d(y+)/dw : the "non-Jerk" dimensions cause the next state to depend
    ...
    for (std::size_t i = 0; i < lpf_torque_ids_.size(); i++) {
      d->Fw.bottomRows(ntau_)(i, lpf_torque_ids_[i]) += double(1 - alpha_);
    }
#endif
    // Jerk partial
    // d->Fw.bottomRows(ntau_).diagonal().array() += double(1 - alpha_);
    state_->JintegrateTransport(y, d->dy, d->Fw, second);

    // d(cost+)/dy
    d->Ly.head(ndx).noalias() = time_step_ * d->differential->Lx;
    // Partial blocks for Jerk dimensions
    ...
    // Partials blocks for non-Jerk dimensions
    d->Lw.setZero();
    d->Lyw.setZero();
    d->Lww.setZero();
#if EIGEN_VERSION_AT_LEAST(3, 4, 0)
    ...
    if (tauReg_weight_ > 0) {
      ...
      d->Lww.diagonal().array()(lpf_torque_ids_) +=
          double(time_step_ * tauReg_weight_);  // tau reg
#else
      for (std::size_t i = 0; i < lpf_torque_ids_.size(); i++) {
    ...
    // Constraint partials for Jerk torque dimensions
  ...
  //           double(time_step_);  // dt*identity top row middle col (eq.
  //                                // Jsecond = d(xnext)/d(dx))
  //       // d->Fy.topLeftCorner(nx, nx).diagonal().array() += double(1.); //
  //       // managed by Jintegrate (eq. Jsecond = d(xnext)/d(dx))
  //       d->Fy.bottomRightCorner(ntau_, ntau_).diagonal().array() =
  //       double(alpha_); d->Fw.topRows(nv).noalias() = da_du * time_step2_ *
  //       (1 - alpha_);
  // #if EIGEN_VERSION_AT_LEAST(3, 4, 0)
  ...
  //       }
  // #endif
  //       d->Fw.bottomRows(nv).diagonal().array() = double(1 - alpha_);
  ...
  //       d->Fy.bottomRightCorner(ntau_, ntau_).diagonal().array() -=
  //           double(1.);  // remove identity from Ftau (due to
  //           stateLPF.Jintegrate)
  ...
  //         d->Lw.noalias() +=
  //             time_step_ * tauLim_weight_ * d->activation->Ar;  // tau lim
  //         d->Lww.diagonal().array() = double(time_step_ * tauReg_weight_); //
  //         reg d->Lww.diagonal() +=
  ...
  //         d->Lww.diagonal().array() += double(tauReg_weight_);  // tau reg
  //         d->Lww.diagonal() +=
...
IntegratedActionModelJerk::createData() {
  return boost::allocate_shared<Data>(Eigen::aligned_allocator<Data>(), this);
}


bool IntegratedActionModelJerk::checkData(
    const boost::shared_ptr<ActionDataAbstract>& data) {
...
IntegratedActionModelJerk::get_differential() const {
  return differential_;
}


const double& IntegratedActionModelJerk::get_dt() const {
  return time_step_;
}


const double& IntegratedActionModelJerk::get_fc() const {
  return fc_;
}


void IntegratedActionModelJerk::set_dt(const double& dt) {
  if (dt < 0.) {
    throw_pretty("Invalid argument: "
...
void IntegratedActionModelJerk::set_fc(const double& fc) {
...
void IntegratedActionModelJerk::set_alpha(const double& alpha) {
  // Set the cut-off frequency
  if (alpha < 0. || alpha > 1) {
    throw_pretty("Invalid argument: "
...
void IntegratedActionModelJerk::compute_alpha(const double& fc) {
...
void IntegratedActionModelJerk::set_differential(
    boost::shared_ptr<DifferentialActionModelAbstract> model) {
...
void IntegratedActionModelJerk::set_control_reg_cost(
...
void IntegratedActionModelJerk::set_control_lim_cost(
    const double& weight) {
  if (weight < 0.) {
    throw_pretty("cost weight is positive ");
  }
...
void IntegratedActionModelJerk::quasiStatic(
