...
ResidualModelFrictionCone::ResidualModelFrictionCone(
    ...
    : Base(state, 1, nu), id_(id), coef_(coef) {
...
ResidualModelFrictionCone::ResidualModelFrictionCone(
    std::shared_ptr<StateMultibody> state, const pinocchio::FrameIndex id,
    const double coef)
    : Base(state, 1), id_(id), coef_(coef) {
  if (static_cast<pinocchio::FrameIndex>(state->get_pinocchio()->nframes) <=
      id) {
    throw_pretty(
        "Invalid argument: "
...
void ResidualModelFrictionCone::calc(
  ...
  data->r[0] =
      coef_ * d->f3d(2) - sqrt(d->f3d(0) * d->f3d(0) + d->f3d(1) * d->f3d(1));
}
...
void ResidualModelFrictionCone::calcDiff(
  ...
  d->f3d = d->contact->f.linear();
  d->dcone_df[0, 0] =
      -d->f3d[0] / sqrt(d->f3d(0) * d->f3d(0) + d->f3d(1) * d->f3d(1));
  d->dcone_df[0, 1] =
      -d->f3d[1] / sqrt(d->f3d(0) * d->f3d(0) + d->f3d(1) * d->f3d(1));
...
ResidualModelFrictionCone::createData(DataCollectorAbstract* const data) {
...
void ResidualModelFrictionCone::print(std::ostream& os) const {
...
pinocchio::FrameIndex ResidualModelFrictionCone::get_id() const { return id_; }
