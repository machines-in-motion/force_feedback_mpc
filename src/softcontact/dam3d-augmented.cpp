...
void DAMSoftContact3DAugmentedFwdDynamics::calc(
  ...
  if (active_contact_) {
    ...
    if (ref_ != pinocchio::LOCAL) {
      d->pinForce =
          pinocchio::ForceTpl<double>(d->oRf.transpose() * f, Vector3s::Zero());
    }
  ...
  if (!active_contact_) {
    if (with_gravity_torque_reg_) {
      ...
      d->residual.segment(this->get_costs()->get_nr(),
                          this->get_state()->get_nv()) = d->tau_grav_residual;
  ...
  if (active_contact_) {
    if (with_force_cost_) {
      ...
      d->residual.segment(
          this->get_costs()->get_nr() + this->get_state()->get_nv(), nc_) =
          d->f_residual;
...
void DAMSoftContact3DAugmentedFwdDynamics::calcDiff(
  ...
  if (active_contact_) {
    ...
    d->dfdt_dx = -(Kp_.asDiagonal() * d->lv_dx.topRows(3)) -
