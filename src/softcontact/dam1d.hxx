...
DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::
  ...
  if (Kp < 0.) {
    throw_pretty("Invalid argument: " << "Kp must be positive ");
  }
  if (Kv < 0.) {
    throw_pretty("Invalid argument: " << "Kv must be positive ");
  }
  Kp_ = Kp;
  Kv_ = Kv;
  oPc_ = oPc;
...
template <typename Scalar>
DifferentialActionModelSoftContact1DFwdDynamicsTpl<
    Scalar>::~DifferentialActionModelSoftContact1DFwdDynamicsTpl() {}
...
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::calc(
  ...
  if (active_contact_) {
    ...
    if (ref_ != pinocchio::LOCAL) {
      d->f3d = -Kp_ * (d->pinocchio.oMf[frameId_].translation() - oPc_) -
               Kv_ * d->oRf * d->lv;
      d->f = d->f3d(this->get_type());
      d->fWORLD = Vector3s::Zero();
      d->fWORLD(this->get_type()) = d->f;
      d->pinForce = pinocchio::ForceTpl<Scalar>(d->oRf.transpose() * d->fWORLD,
                                                Vector3s::Zero());
      d->fext[parentId_] = jMf_.act(d->pinForce);
    }
...
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::calcDiff(
  ...
  if (active_contact_) {
    ...
    if (ref_ != pinocchio::LOCAL) {
      d->df3d_dx.leftCols(nv) =
          d->oRf * d->df3d_dx_copy.leftCols(nv) -
          pinocchio::skew(d->oRf * d->f3d_copy) * d->oJ.bottomRows(3);
    ...
    if (ref_ == pinocchio::LOCAL) {
      d->Fx.leftCols(nv) =
          d->aba_dq + d->aba_dtau *
                          d->lJ.topRows(3).transpose().col(this->get_type()) *
      ...
      d->Fx.leftCols(nv) =
          d->aba_dq + d->aba_dtau * d->lJ.topRows(3).transpose() *
                          (d->oRf.transpose().col(this->get_type()) *
                               d->df_dx.leftCols(nv) +
                           pinocchio::skew(d->oRf.transpose() * d->fWORLD) *
                               d->lJ.bottomRows(3));
      d->Fx.rightCols(nv) =
          d->aba_dv + d->aba_dtau * d->lJ.topRows(3).transpose() *
                          d->oRf.transpose().col(this->get_type()) *
...
template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_force_cost(
    const Scalar force_des, const Scalar force_weight) {
  if (force_weight < 0.) {
    throw_pretty("Invalid argument: " << "Force weight should be positive");
  }
...
template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_Kp(
    const Scalar inKp) {
  if (inKp < 0.) {
...
template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_Kv(
    const Scalar inKv) {
  if (inKv < 0.) {
...
template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_oPc(
...
template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_force_des(
    const Scalar inForceDes) {
  force_des_ = inForceDes;
}

template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<
    Scalar>::set_force_weight(const Scalar inForceWeight) {
  if (inForceWeight < 0.) {
    throw_pretty(
        "Invalid argument: " << "Force cost weight should be positive");
  }
  force_weight_ = inForceWeight;
}

template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_ref(
    const pinocchio::ReferenceFrame inRef) {
  ref_ = inRef;
}

template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_id(
...
template <typename Scalar>
const Scalar
DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::get_Kp() const {
  return Kp_;
}

template <typename Scalar>
const Scalar
DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::get_Kv() const {
  return Kv_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector3s&
DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::get_oPc() const {
  return oPc_;
}

template <typename Scalar>
const Scalar DifferentialActionModelSoftContact1DFwdDynamicsTpl<
    Scalar>::get_force_des() const {
  return force_des_;
}
...
DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::get_ref() const {
  return ref_;
}

template <typename Scalar>
const pinocchio::FrameIndex&
DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::get_id() const {
  return frameId_;
}

template <typename Scalar>
void DifferentialActionModelSoftContact1DFwdDynamicsTpl<Scalar>::set_type(
