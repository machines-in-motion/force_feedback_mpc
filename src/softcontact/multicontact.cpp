...
pinocchio::Force ViscoElasticContact3D::calc(pinocchio::Data& data,
  ...
  if (pinRef == pinocchio::LOCAL) {
    ...
    fext =
        jMf.act(pinocchio::Force(oRf.transpose() * f, Eigen::Vector3d::Zero()));
  }
...
Eigen::Vector3d ViscoElasticContact3D::calc_fdot(pinocchio::Data& data) {
  pinocchio::Motion v =
      pinocchio::getFrameVelocity(model, data, frameId, pinocchio::LOCAL);
  pinocchio::Motion a =
      pinocchio::getFrameAcceleration(model, data, frameId, pinocchio::LOCAL);

  fout = -Kp * v.linear() - Kv * a.linear();
  fout_copy = fout;

  if (pinRef != pinocchio::LOCAL) {
    pinocchio::Motion v_world = pinocchio::getFrameVelocity(
        model, data, frameId, pinocchio::LOCAL_WORLD_ALIGNED);
    pinocchio::Motion a_world = pinocchio::getFrameAcceleration(
        model, data, frameId, pinocchio::LOCAL_WORLD_ALIGNED);
...
void ViscoElasticContact3D::update_ABAderivatives(pinocchio::Data& data,
                                                  const Eigen::Vector3d& f) {
  Eigen::MatrixXd lJ =
      pinocchio::getFrameJacobian(model, data, frameId, pinocchio::LOCAL);
  pinocchio::SE3 oMf = data.oMf[frameId];
  Eigen::Matrix3d oRf = oMf.rotation();

  dABA_df = data.Minv * lJ.topRows(3).transpose() *
            model.frames[frameId].placement.rotation() *
            Eigen::Matrix3d::Identity();

  if (pinRef != pinocchio::LOCAL) {
    Eigen::Matrix3d f_skew = pinocchio::skew(oRf.transpose() * f);
    data.Fx.topRows(model.nv) +=
        data.Minv * lJ.topRows(3).transpose() * f_skew * lJ.bottomRows(3);
...
void ViscoElasticContact3D::calcDiff(pinocchio::Data& data) {
  ...
  pinocchio::getFrameVelocityDerivatives(model, data, frameId, pinocchio::LOCAL,
                                         v_dq, v_dv);

  Eigen::MatrixXd a_dq, a_dv, a_da;
  pinocchio::getFrameAccelerationDerivatives(
      model, data, frameId, pinocchio::LOCAL, a_dq, a_dv, a_da);

  Eigen::MatrixXd da_dx = Eigen::MatrixXd::Zero(3, 2 * model.nv);
  da_dx.leftCols(model.nv) =
      a_dq.topRows(3) + a_da.topRows(3) * data.Fx.topRows(model.nv);
  da_dx.rightCols(model.nv) =
      a_dv.topRows(3) + a_da.topRows(3) * data.Fx.bottomRows(model.nv);
  ...
  if (pinRef != pinocchio::LOCAL) {
    Eigen::MatrixXd oJ = pinocchio::getFrameJacobian(
        model, data, frameId, pinocchio::LOCAL_WORLD_ALIGNED);
    dfdt_dx.leftCols(model.nv) =
        oRf * ldfdt_dx_copy.leftCols(model.nv) -
        pinocchio::skew(oRf * fout_copy) * oJ.bottomRows(3);
...
ViscoElasticContact3dMultiple::ViscoElasticContact3dMultiple(
...
std::vector<pinocchio::Force> ViscoElasticContact3dMultiple::calc(
    pinocchio::Data& data, const Eigen::VectorXd& f) {
  int nc_i = 0;
...
Eigen::VectorXd ViscoElasticContact3dMultiple::calc_fdot(
    pinocchio::Data& data) {
  int nc_i = 0;
  fout.setZero();
...
void ViscoElasticContact3dMultiple::update_ABAderivatives(
    pinocchio::Data& data, const Eigen::VectorXd& f) {
  int nc_i = 0;
...
void ViscoElasticContact3dMultiple::calcDiff(pinocchio::Data& data) {
  ...
  for (auto& ct : contacts) {
    if (ct->isActive()) {
      ...
      data.dfdt_df.block(nc_i, nc_i, ct->getNc(), ct->getNc()) =
          ct->get_dfdt_df();
    }
    nc_i += ct->getNc();
  }
...
FrictionConeConstraint::FrictionConeConstraint(pinocchio::FrameIndex frameId,
                                               double coef)
...
ForceConstraintManager::ForceConstraintManager(
    const std::vector<std::shared_ptr<FrictionConeConstraint>>&
        frictionConstraints,
  ...
  for (auto& fc : frictionConstraints) {
    ...
    if (!found) {
      throw std::runtime_error(
          "No contact model found for friction constraint on frame " +
          std::to_string(fc->getFrameId()));
    }
  }
  ...
  for (auto& bc : boxConstraints) {
    ...
    if (!found) {
      throw std::runtime_error(
          "No contact model found for box constraint on frame " +
          std::to_string(bc->getFrameId()));
    }
  }
  ...
  has_force_constraint =
      !frictionConstraints.empty() || !boxConstraints.empty();
...
Eigen::VectorXd ForceConstraintManager::calc(const Eigen::VectorXd& f) {
  ...
  for (auto& ct : contacts->getContacts()) {
    ...
    for (auto& fc : contact_to_friction_map[ct->getFrameId()]) {
      residual.segment(nr_i, fc->getNr())
          << fc->calc(f.segment(nc_i, fc->getNc()));
      nr_i += fc->getNr();
    }

    // Process box constraints
    for (auto& bc : contact_to_box_map[ct->getFrameId()]) {
      residual.segment(nr_i, bc->getNr()) =
          bc->calc(f.segment(nc_i, bc->getNc()));
      nr_i += bc->getNr();
    }
...
Eigen::MatrixXd ForceConstraintManager::calcDiff(const Eigen::VectorXd& f) {
  ...
  for (auto& ct : contacts->getContacts()) {
    ...
    // Process friction constraints
    for (auto& fc : contact_to_friction_map[ct->getFrameId()]) {
      residual_df.block(nr_i, nc_i, fc->getNr(), fc->getNc()) =
          fc->calcDiff(f.segment(nc_i, fc->getNc()));
      nr_i += fc->getNr();
    }

    // Process box constraints
    for (auto& bc : contact_to_box_map[ct->getFrameId()]) {
      residual_df.block(nr_i, nc_i, bc->getNr(), bc->getNc()) =
          bc->calcDiff(f.segment(nc_i, bc->getNc()));
      nr_i += bc->getNr();
    }
...
ForceCost::ForceCost(pinocchio::Model& model, pinocchio::FrameIndex frameId,
...
double ForceCost::calc(pinocchio::Data& data, const Eigen::Vector3d& f,
                       pinocchio::ReferenceFrame pinRefDyn) {
  pinocchio::SE3 oMf = data.oMf[frameId];
  Eigen::Matrix3d oRf = oMf.rotation();
...
std::pair<Eigen::Vector3d, Eigen::Matrix3d> ForceCost::calcDiff(
    pinocchio::Data& data, const Eigen::Vector3d& f,
    pinocchio::ReferenceFrame pinRefDyn) {
  pinocchio::SE3 oMf = data.oMf[frameId];
  Eigen::Matrix3d oRf = oMf.rotation();
  Eigen::MatrixXd lJ =
  ...
  if (pinRef != pinRefDyn) {
    if (pinRef == pinocchio::LOCAL) {
      ...
      f_residual_x.leftCols(model.nv) =
          pinocchio::skew(oRf.transpose() * f) * lJ.bottomRows(3);
      ...
      Eigen::MatrixXd oJ = pinocchio::getFrameJacobian(
          model, data, frameId, pinocchio::LOCAL_WORLD_ALIGNED);
      f_residual_x.leftCols(model.nv) =
          pinocchio::skew(oRf * f) * oJ.bottomRows(3);
      data.Lx += f_residual.transpose() * f_weight * pinocchio::skew(oRf * f) *
                 f_residual_x;
...
ForceCostManager::ForceCostManager(
    const std::vector<std::shared_ptr<ForceCost>>& forceCosts,
    const std::shared_ptr<ViscoElasticContact3dMultiple>& contacts)
  ...
  for (auto& fc : forceCosts) {
    ...
    if (!found) {
      throw std::runtime_error(
          "No contact model found for force cost on frame " +
          std::to_string(fc->getFrameId()));
    }
  }
...
std::pair<Eigen::VectorXd, Eigen::MatrixXd> ForceCostManager::calcDiff(
    pinocchio::Data& data, const Eigen::VectorXd& f) {
  int nc_i = 0;
  Lf.setZero();
  Lff.setZero();
  ...
  for (auto& ct : contacts->getContacts()) {
    ...
    for (auto& cost : contact_to_cost_map[ct->getFrameId()]) {
      auto [Lf_ct, Lff_ct] =
          cost->calcDiff(data, f.segment(nc_i, cost->getNc()), pinRefDyn);
